...
- [Prevenzione della perdita dei dati di Microsoft 365 Endpoint](../compliance/endpoint-dlp-learn-about.md)
- [Office 365 Message Encryption new capabilities (OME)](../compliance/ome.md) e OME [Advanced Message Encryption](../compliance/ome-advanced-message-encryption.md)
...
1. Comprendere quanto segue:
   ...
   - **Aspetti chiave da ricordare** Le etichette di riservatezza vengono gestite nell'interfaccia di amministrazione di Conformità Microsoft, ma le opzioni di destinazione e applicazione variano in modo significativo.
      ...
      - Esistono etichette di riservatezza per l'applicazione automatica ai file in pausa in SharePoint e OneDrive oltre ai messaggi di posta elettronica in transito tramite Exchange. Sono destinati a tutti i siti o a quelli specifici e si applicano automaticamente ai file in fase di riposo in questi ambienti.
...
Questo metodo è denominato classificazione automatica con etichette di riservatezza. Potresti anche sentirla come etichettatura automatica per i dati in pausa (per i documenti in SharePoint e OneDrive) e i dati in transito (per i messaggi di posta elettronica inviati o ricevuti da Exchange). Per Exchange, non include messaggi di posta elettronica nelle cassette postali in pausa.
...
Per istruzioni sulla configurazione, [vedere Come configurare i criteri di etichettatura automatica per SharePoint, OneDrive ed Exchange.](../compliance/apply-sensitivity-label-automatically.md#how-to-configure-auto-labeling-policies-for-sharepoint-onedrive-and-exchange)
...
Ad esempio, è possibile identificare qualsiasi documento contenente un numero di carta di credito archiviato in qualsiasi sito di OneDrive oppure è possibile monitorare solo i siti di OneDrive di persone specifiche.
...
| Etichettatura automatica <br> L'organizzazione vuole proteggere automaticamente la posta/allegati con contenuti sensibili specifici e/o destinatari specifici | Recommend (E5) - Auto-labeling in Exchange and Outlook clients, augments mail flow rules and DLP policy | Sì - Regole del flusso di posta e criteri DLP con protezione Solo crittografia o Non inoltrare |
||||

Esistono anche differenze nelle esperienze di amministratore e utente finale tra questi due metodi.

## <a name="teams-with-protection-for-highly-sensitive-data"></a>Teams con protezione per dati estremamente sensibili
