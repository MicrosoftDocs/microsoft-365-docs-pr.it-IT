---
title: GDPR semplificato una guida per le piccole imprese
f1.keywords:
- NOCSH
ms.author: pebaum
author: pebaum
...
GDPR è una norma europea che aggiorna ed espande la prima direttiva sulla protezione dei dati (DPD) prima emanata nel 1995. La GDPR è interessata alla privacy dei dati di un singolo utente, sia che si tratti di un cliente, di un cliente, di un dipendente o di un partner aziendale. L'obiettivo di GDPR è rafforzare la protezione dei dati personali per i cittadini dell'Unione europea, sia che risiedano nell'Unione europea o altrove. Il regolamento definisce le aspettative e fornisce consigli su come raggiungerli. Quando la GDPR diventa esecutiva alla fine di maggio 2018, le organizzazioni devono disporre di misure in grado di soddisfare i requisiti del GDPR.
  
Il GDPR è tutto sui dati e su come viene utilizzato. Considerare i dati come aventi un ciclo di vita. Il ciclo inizia quando si raccolgono dati, continua mentre lo si archivia e lo utilizza (elaborazione) e termina quando viene eliminato completamente dai sistemi. 
  
GDPR si occupa dei tipi di dati seguenti: 
  
- **Dati personali:** Se è possibile collegare i dati a un singolo utente e identificarli, i dati vengono considerati personali rispetto a GDPR. Esempi di dati personali includono nome, indirizzo, data di nascita e indirizzo IP. Il GDPR Considera anche informazioni codificate (note anche come "pseudonimo") come dati personali, indipendentemente dal modo in cui i dati sono oscuri o tecnici, se i dati possono essere collegati a un singolo utente.
    
- **Dati personali sensibili** Si tratta di dati che aggiungono ulteriori dettagli ai dati personali. Tra gli esempi sono inclusi la religione, l'appartenenza al sindacato, l'origine etnica e così via. I dati personali sensibili includono anche dati biometrici e DNA. In GDPR, i dati sensibili dispongono di regole di protezione più rigorose rispetto ai dati personali. 
    
## <a name="gdpr-terms"></a>Termini di GDPR

In GDPR verranno visualizzati alcuni termini riferiti frequentemente. È importante comprendere questi termini.
...
- **Essere informati sui loro dati:** È necessario informare gli utenti sull'utilizzo dei dati. 
    
- **Avere accesso ai propri dati:** È necessario fornire agli utenti l'accesso a tutti i dati che si contengono (ad esempio, utilizzando l'accesso agli account o in qualche modo manuale). 
    
- **Richiedere la rettifica dei dati:** Gli utenti possono richiedere la correzione dei dati non accurati. 
    
- **Richiedere l'eliminazione dei dati:** Conosciuto anche come ' right to cancellation ', questo diritto consente a un singolo utente di richiedere che tutti i dati personali raccolti da una società vengano eliminati in tutti i sistemi che lo utilizzano o lo condividono. 
    
- **Richieste di elaborazione limitate:** Un singolo utente può richiedere di sopprimere o limitare i dati. Tuttavia, è applicabile solo in determinate circostanze. 
    
- **Avere la portabilità dei dati:** Un singolo utente può richiedere il trasferimento dei dati a un'altra società. 
    
- **Oggetto:** Un singolo utente può opporsi ai dati utilizzati per vari usi, tra cui direct marketing. 
    
- **Chiedere di non essere soggetti al processo decisionale automatizzato, inclusa la profilatura:** Le regole di GDPR sono rigorose sull'utilizzo dei dati per il profilo di persone e automatizzare le decisioni basate su tale profiling. 
    
## <a name="common-small-business-questions-about-the-gdpr"></a>Domande comuni sulle piccole imprese relative all'GDPR

### <a name="im-a-sole-proprietor---do-i-really-have-to-worry-about-the-gdpr"></a>Sono un proprietario unico: devo davvero preoccuparmi del GDPR?

La GDPR è relativa ai dati elaborati, non al numero di dipendenti. Interessa le aziende di tutte le dimensioni, anche i proprietari unici. Tuttavia, le aziende con meno di 250 dipendenti hanno alcune esenzioni, ad esempio la riduzione dei record, ma solo se si è certi che il trattamento dei dati non influisce sui diritti individuali ed è un'elaborazione occasionale.
  
Ad esempio, l'elaborazione di dati non personali verrebbe esente o avrà bisogno di misure ridotte. Tuttavia, se si elaborano dati che vengono visualizzati come "dati riservati di categorie speciali", anche se solo occasionalmente, sarà necessario registrare l'elaborazione dei dati. La definizione di "elaborazione occasionale" è vaga, ma è destinata a essere applicata ai dati che vengono utilizzati una o più volte.
  
È inoltre necessario verificare che i dati personali raccolti siano protetti. Questo significa che è necessario crittografarlo e verificare che l'accesso venga controllato utilizzando almeno una password. Mantenere i dati dei clienti su un foglio di calcolo sul desktop senza alcuna protezione non soddisferà le aspettative di GDPR. 
  
### <a name="how-can-i-tell-if-our-company-website-is-gdpr-compliant"></a>Come si fa a sapere se il sito Web aziendale è GDPR conforme?
...
La GDPR è una regolamentazione che protegge i cittadini dell'Unione europea. Se la tua azienda si occupa dei cittadini dell'Unione europea adesso, o speri di riuscire in futuro, sarai intaccata. Ciò vale sia per i cittadini che vivono in uno stato dell'Unione europea che per coloro che vivono altrove.
  
Si considerino gli esempi seguenti:
  
- Una società statunitense che noleggia automobili ai cittadini dell'Unione dovrà soddisfare i requisiti di GDPR quando raccolgono ed elaborano i dati del cliente. La società dovrà prendere il consenso quando prende i dati del cliente e verificare che i dati vengano archiviati in modo sicuro. Sarà inoltre necessario assicurarsi che il cliente possa applicare tutti i diritti del soggetto dei dati. 
    
- Una società australiana vende prodotti online e i suoi utenti configurano gli account online. I diritti e il consenso del soggetto dei dati di GDPR verranno applicati ai cittadini dell'Unione europea che aprono un account. La società dovrà assicurarsi che il cliente possa applicare tutti i diritti del soggetto dei dati. 
...
In GDPR, se si raccolgono dati in una certa misura. Il GDPR ha il concetto di un Data Processor e di un titolare del trattamento dei dati: 
...
È necessario assicurarsi che i clienti, anche quelli che si sono verificati da anni, abbiano acconsentito a utilizzare i dati per la commercializzazione. Potrebbe essere stato precedentemente acquisito il consenso, oltre a un record per visualizzarlo. In caso affermativo, è tutto pronto per continuare il marketing. In caso contrario, è necessario ottenere l'autorizzazione da parte del cliente per continuare a commercializzarli. Questo solitamente implica l'invio di un messaggio di posta elettronica per chiedere ai clienti di accedere al sito e selezionare un'opzione per consentire la ricezione di messaggi di posta elettronica futuri. 
  
### <a name="do-i-have-to-worry-about-the-gdpr-when-i-recruit-new-employees-what-about-current-employees"></a>È necessario preoccuparsi del GDPR quando si reclutano nuovi dipendenti? Informazioni sui dipendenti correnti

Il GDPR non incide solo sui dati dei clienti; si estende anche ai dati dei dipendenti. Le nuove reclute sono spesso situate usando piattaforme social media come LinkedIn. Assicurarsi di non archiviare eventuali dati di reclutamento potenziali senza l'autorizzazione espressa.
...
Per prepararsi per l'GDPR, ecco alcuni suggerimenti per i passaggi successivi da eseguire:
...
Blog ufficiale Microsoft: [impegno di Microsoft per GDPR](https://blogs.microsoft.com/on-the-issues/2018/05/21/microsofts-commitment-to-gdpr-privacy-and-putting-customers-in-control-of-their-own-data/)
  
Siti della Commissione europea:
  
- [Protezione dei dati](https://ec.europa.eu/info/law/law-topic/data-protection)
    
- [2018 riforma delle regole di protezione dei dati dell'Unione europea](https://ec.europa.eu/commission/priorities/justice-and-fundamental-rights/data-protection/2018-reform-eu-data-protection-rules)
