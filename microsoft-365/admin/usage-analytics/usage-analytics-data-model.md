...
## <a name="data-for-the-microsoft-365-usage-analytics-tables"></a>Dati per le tabelle di analisi dell'utilizzo di Microsoft 365

L'analisi dell'utilizzo di Microsoft 365 si connette a un'API che espone un modello di dati multidimensionali. Le API sono in anteprima ed è possibile accedervi (sostituire il con `https://reports.office.com/pbi/v1.0/\<tenantid\>` il GUID del \<tenant id\> tenant). 
  
> [!NOTE]
> Per ulteriori informazioni, vedere [Utilizzo dei report sull'utilizzo di Microsoft 365 in Microsoft Graph.](https://go.microsoft.com/fwlink/p/?linkid=864336) 
...
## <a name="data-tables-returned-by-the-microsoft-365-reporting-api"></a>Tabelle di dati restituite dall'API per la creazione di report di Microsoft 365
...
|Tenant Office Licenses  <br/> |Contiene i dati sul numero di abbonamenti a Microsoft Office assegnati agli utenti  <br/> |Contiene i dati sullo stato di fine mese per un periodo di 12 mesi in sequenza, incluso il mese parziale corrente.  <br/> |
|Tenant Mailbox Usage  <br/> |Contiene i dati sulla cassetta postale dell'utente, per il numero totale di cassette postali e la modalità di utilizzo dell'archiviazione.  <br/> |Contiene i dati sullo stato di fine mese per un periodo di 12 mesi in sequenza, incluso il mese parziale corrente.  <br/> |
...
|Tenant SharePoint Online Usage  <br/> |Contiene i dati relativi ai siti di SharePoint, inclusi i siti del gruppo o del team, ad esempio il numero totale dei siti, il numero di documenti del sito, il numero di file per tipo di attività e lo spazio di archiviazione usato.  <br/> |Contiene i dati sullo stato di fine mese per un periodo di 12 mesi in sequenza, incluso il mese parziale corrente.  <br/> |
|Tenant OneDrive for Business Usage  <br/> |Contiene i dati relativi gli account di OneDrive, ad esempio il numero di account, il numero di documenti nelle varie istanze di OneDrive, lo spazio di archiviazione usato e il numero di file per tipo di attività.  <br/> |Contiene i dati sullo stato di fine mese per un periodo di 12 mesi in sequenza, incluso il mese parziale corrente.  <br/> |
|Tenant Microsoft 365 Groups Usage  <br/> |Contiene i dati sull'utilizzo dei gruppi di Microsoft 365, tra cui Cassette postali, SharePoint e Yammer.  <br/> |Contiene i dati sullo stato di fine mese per un periodo di 12 mesi in sequenza, incluso il mese parziale corrente.  <br/> |
|Tenant Office Activation  <br/> |Contiene i dati relativi al numero di attivazioni dell'abbonamento a Office, al numero di attivazioni per dispositivo (Android/iOS/Mac/PC), alle attivazioni in base al piano di servizio, ad esempio Microsoft 365 Apps for enterprise, Visio, Project.  <br/> |Contiene i dati sullo stato di fine mese per un periodo di 12 mesi in sequenza, incluso il mese parziale corrente.  <br/> |
...
Questa tabella fornisce i dettagli a livello di utente per tutti gli utenti a cui è assegnata una licenza durante l'ultimo mese completo. I dati vengono importati da Azure Active Directory.
...
|IDType  <br/> |Il tipo di ID è impostato su 1 se l'utente è un utente di Yammer che si connette utilizzando il proprio ID Yammer o 0 se si connette a Yammer usando il proprio ID Microsoft 365.  <br/> Il valore è 1 per rappresentare che questo utente si connette a Yammer con l'ID Yammer e non con l'ID di Microsoft 365  <br/> |
...
|HasLicenseTeams  <br/> |Impostare su true se all'utente è assegnata una licenza e abilitare l'uso di Microsoft Teams.  <br/> |
...
|YAM_State  <br/> |Gli stati dell'utente nel sistema Yammer possono essere attivi, eliminati o sospesi.  <br/> |
...
|EXO_EmailRead  <br/> |Numero di e-mail di attività di lettura eseguite dall'utente, potrebbe essere più volte la lettura di un messaggio di posta elettronica già letto o un messaggio di posta elettronica ricevuto in precedenza.  <br/> |
...
|ODB_FileSharedInternally  <br/> |Numero di file condivisi dall'utente internamente da OneDrive for Business o con utenti all'interno di gruppi (che potrebbero includere utenti esterni).  <br/> |
...
|SPO_GroupFileSharedInternally  <br/> |Il numero di file condivisi con gli utenti all'interno dell'organizzazione o con gli utenti all'interno dei gruppi (che potrebbero includere utenti esterni).  <br/> |
...
|SPO_OtherFileSharedExternally  <br/> |Numero di file condivisi dall'utente esternamente da qualsiasi altro sito.  <br/> |
...
|SPO_OtherAccessedByOthers  <br/> |Numero di siti con cui l'utente ha interagito che risiedono in un altro sito di cui è proprietario un altro utente.  <br/> |
...
|SPO_TeamFileSharedInternally  <br/> |Numero di file che l'utente ha condiviso internamente da qualsiasi sito del team o con utenti all'interno di gruppi (che potrebbero includere utenti esterni).  <br/> |
...
|Teams_ChannelMessage  <br/> |Numero di messaggi inseriti nei canali.  <br/> |
|Teams_CallParticipate  <br/> |Numero di chiamate a cui l'utente ha partecipato.  <br/> |
|Teams_MeetingParticipate  <br/> |Numero di riunioni a cui l'utente si è unito.  <br/> |
...
|YAM_MessageLiked  <br/> |Numero di messaggi di Yammer apprezzati dall'utente.  <br/> |
...
> [!NOTE]
> Teams_HasOtherAction indica che l'utente è considerato attivo, ma ha un valore pari a zero per i messaggi di chat, le chiamate 1:1, i messaggi di canale, il totale delle riunioni e le riunioni organizzate.
   
### <a name="data-table---tenant-product-usage"></a>Tabella di dati - Tenant Product Usage

Questa tabella fornisce i dati sull'adozione mensile in termini di utenti abilitati, attivi, di ritorno e di primo utilizzo per ogni prodotto in Microsoft 365. I valori di Microsoft 365 rappresentano l'utilizzo attivo in uno dei prodotti.
  
|**Nome colonna**|**Descrizione della colonna**|
|:-----|:-----|
|Product  <br/> |Nomi dei prodotti per cui vengono riepilogate le informazioni sull'utilizzo. Il valore di Microsoft 365 nella colonna del prodotto rappresenta l'attività in tutti i prodotti  <br/> |
...
|EnabledUsers  <br/> |Numero di utenti abilitati a usare il prodotto per il valore dell'intervallo di tempo, se un utente è stato abilitato per parte del mese, viene comunque conteggiato.  <br/> |
...
|:-----|:-----|
|Timeframe  <br/> |Valore del mese. Ci sarà una riga per ogni prodotto al mese per gli ultimi 12 mesi tra cui il mese corrente parziale.  <br/> |
|Prodotto  <br/> |Nome del prodotto in Microsoft 365 per cui sono disponibili i dati di utilizzo.  <br/> |
...
Questa tabella è costituita da dati di riepilogo per tutti gli utenti di Exchange Online con licenza che dispongono di una cassetta postale utente. Contiene lo stato alla fine del mese per tutte le cassette postali utente. I dati in questa tabella non sono additivi tra i vari mesi. I dati dell'ultimo mese in questa tabella rappresentano lo stato più recente.
...
Questa tabella fornisce dati di riepilogo mensili sui client che gli utenti usano per connettersi a Exchange Online, Skype for Business e Yammer. Questa tabella non contiene ancora dati sull'utilizzo client per SharePoint Online e OneDrive for Business.
  
|**Nome colonna**|**Descrizione della colonna**|
|:-----|:-----|
|Product  <br/> |Nome del prodotto in Microsoft 365 per cui sono disponibili i dati di utilizzo client.  <br/> |
...
Questa tabella contiene i dati di riepilogo mensili relativi all'utilizzo o alle attività dei siti di SharePoint Online. Copre solo i siti del team e del gruppo. Lo stato di fine mese dei siti di SharePoint Online è rappresentato in questa colonna, ad esempio, se un utente ha creato cinque documenti e ha utilizzato 10 MB per l'archiviazione totale, quindi ha eliminato alcuni file e aggiunto altri file in modo che alla fine del mese lo stato per i file sia pari a sette in totale che utilizzano cinque MB di spazio di archiviazione, il valore rappresentato in questa tabella è lo stato di fine mese. Questa tabella è nascosta per evitare i conteggi duplicati delle aggregazioni e viene usata come origine per creare due tabelle di riferimento.
...
|SitesWithOwnerActivities  <br/> |Numero dei siti attivi, in cui il proprietario del sito ha eseguito una specifica attività sui file dei propri siti. È possibile ottenere il proprietario del sito dal comando di PowerShell **get-un tempo.** Si tratta della persona responsabile del sito.   <br/> |
|SitesWithNonOwnerActivities  <br/> |Somma dei siti attivi per il mese specificato, in cui gli utenti non proprietari del sito hanno eseguito una specifica attività sui file dei siti. È possibile ottenere il proprietario del sito dal comando di PowerShell **get-un tempo.** Si tratta della persona responsabile del sito. <br/> |
...
### <a name="data-table---tenant-onedrive-usage"></a>Tabella dati - Uso di OneDrive tenant

Questa tabella fornisce i dati sugli account di OneDrive, come il numero di account, il numero di documenti in tutti gli account di OneDrive, lo spazio di archiviazione usato, il numero di file per tipo di attività. Lo stato alla fine del mese degli account di OneDrive for Business viene rappresentato in questa tabella. Ad esempio, se un utente ha creato cinque documenti che hanno utilizzato 10 MB di spazio di archiviazione e quindi ha eliminato alcuni file e aggiunto altri file in modo che alla fine del mese abbia sette file che utilizzano cinque MB di spazio di archiviazione, il valore di fine mese viene rappresentato in questa tabella alla fine del mese.
...
|Diplansed  <br/> |Spazio di archiviazione totale usato sommato in tutti gli account di OneDrive alla fine dell'intervallo di tempo.  <br/> |
...
### <a name="data-table---tenant-microsoft-365-groups-usage"></a>Tabella dati - Tenant Microsoft 365 Groups Usage

Questa tabella fornisce dati sul modo in cui i gruppi di Microsoft 365 vengono usati nell'organizzazione.
...
|MBX_TotalActivities  <br/> |Numero di attività della cassetta postale.  <br/> |
|MBX_TotalItems  <br/> |Numero di elementi della cassetta postale.  <br/> |
|MBX_StorageUsed  <br/> |Quantità di spazio di archiviazione della cassetta postale utilizzato.  <br/> |
|SPO_TotalGroups  <br/> |Numero di gruppi di SharePoint.  <br/> |
|SPO_ActiveGroups  <br/> |Numero di gruppi di SharePoint attivi.  <br/> |
|SPO_FileAccessedActiveGroups  <br/> |Numero di gruppi di SharePoint con attività di accesso ai file.  <br/> |
|SPO_FileSyncedActiveGroups  <br/> |Numero di gruppi di SharePoint con attività di sincronizzazione dei file.  <br/> |
|SPO_FileSharedInternallyActiveGroups  <br/> |Numero di gruppi di SharePoint che hanno attività condivise internamente o con gruppi (che possono includere utenti esterni).  <br/> |
...
|SPO_FileSharedInternallyActivities  <br/> |Numero di attività condivise di file di SharePoint internamente o con gruppi (che potrebbero includere membri esterni).  <br/> |
|SPO_FileSharedExternallyActivities  <br/> |Numero di attività di file di SharePoint condivise esternamente.  <br/> |
...
|YAM_PostedActiveGroups  <br/> |Numero di gruppi di Yammer con attività di post.  <br/> |
|YAM_ReadActiveGroups  <br/> |Numero di gruppi di Yammer con attività di lettura.  <br/> |
|YAM_TotalActivities  <br/> |Numero di attività di Yammer.  <br/> |
|YAM_LikedActivities  <br/> |Numero di attività come Yammer.  <br/> |
|YAM_PostedActivties  <br/> |Numero di attività post di Yammer.  <br/> |
...
La tabella fornisce i dati sul numero di attivazioni dell'abbonamento a Office nei piani di servizio, ad esempio Microsoft 365 Apps for enterprises, Visio, Project. Fornisce anche i dati sul numero di attivazioni per ogni dispositivo (Android/iOS/Mac/PC).
