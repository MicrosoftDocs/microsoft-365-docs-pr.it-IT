...
description: Informazioni su come verificare il dominio e configurare i record DNS per posta elettronica, Skype for Business online e altri servizi in DNS basato su Windows per Microsoft.
...
Problemi con il flusso di posta o altri problemi dopo l'aggiunta di record DNS, vedere Risolvere i problemi dopo aver modificato il nome [di dominio o i record DNS.](../get-help-with-domains/find-and-fix-issues.md) 
  
## <a name="find-your-dns-records-in-windows-based-dns"></a>Trovare i record DNS in un server DNS basato su Windows
<a name="BKMK_find_your_dns_1"></a> Passare alla pagina contenente i record DNS per il dominio. Se si lavora in Windows Server 2008, passare a **Avvia**  >  **Esecuzione.** Se si lavora in Windows Server 2012, premere il tasto Windows e **r**. Digitare **dnsmgmnt.msc** e quindi selezionare **OK.** In Gestore DNS espandere **\<DNS server name\> \> Zone di ricerca diretta.** Selezionare il dominio. A questo punto è possibile creare i record DNS.
...
- Il record MX da aggiungere include  un valore (il valore indirizzo di puntamento) simile al seguente: .mail.protection.outlook.com, dove è un valore come \<MX token\> \<MX token\> MSxxxxxxx. 
...
- Nella pagina Gestore DNS per il dominio, andare a **Action**  >  **Mail Exchanger (MX)**. Per trovare questa pagina per il dominio, vedere [Trovare i record DNS in DNS basato su Windows.](#find-your-dns-records-in-windows-based-dns)  
- Nella finestra **di dialogo Nuovo record** di risorse verificare che i campi siano impostati esattamente sui valori seguenti: 
    - Nome host:  
    - @Address: incollare il valore dell'indirizzo di Puntamento appena copiato da Microsoft qui.  
    - Pref: 
...
Aggiungere i record CNAME necessari per Microsoft. Se in Microsoft sono elencati altri record CNAME, aggiungerli seguendo la stessa procedura generale illustrata di seguito.
...
- Nella pagina Gestore DNS per il dominio, andare ad **Azione**  >  **CNAME (CNAME)**.
- Nella finestra **di dialogo Nuovo record** di risorse verificare che i campi siano impostati esattamente sui valori seguenti:  
...
- Nella pagina Gestore DNS per il dominio, andare ad **Azione** \> **CNAME (CNAME)**. Nella finestra **di dialogo Nuovo record** di risorse verificare che i campi siano impostati esattamente sui valori seguenti:  
    - Nome host: lyncdiscover
    - Tipo: CNAME
    - Indirizzo: webdir.online.lync.com
- Selezionare **OK**.
   
### <a name="add-two-cname-records-for-mobile-device-management-mdm-for-microsoft"></a>Aggiungere due record CNAME per Gestione di dispositivi mobili (MDM) per Microsoft
...
-  Nella finestra **di dialogo Nuovo record** di risorse verificare che i campi siano impostati esattamente sui valori seguenti. 
 > [!IMPORTANT]
> In alcune versioni di Gestione DNS di Windows, è possibile che il dominio sia stato configurato in modo che, quando si crea un record txt, il nome principale sia quello predefinito del dominio padre. In questo caso, quando si aggiunge un record TXT, impostare il nome host su vuoto (nessun valore) invece che su @ o sul nome di dominio. 
...
-  Nella pagina Dns Manager del dominio passare ad **Action** \> **Other New Records.** 
...
3. Nella colonna **Azioni** del  dominio che si sta verificando nella pagina Domini selezionare **Avvia configurazione.** 
4. Nella pagina **Aggiungi un dominio a Microsoft** selezionare Il passaggio iniziale **1.** 
5. Nella pagina **Confermare di essere** proprietari  del dominio, nell'elenco a discesa Vedere le istruzioni per eseguire questo passaggio con, scegliere **Istruzioni generali.** 
...
-  Nell'area **Custom Host Names** della finestra di dialogo New Resource **Record** verificare che i campi siano impostati esattamente sui valori seguenti. 
...
- Tornare a Microsoft e seguire la procedura seguente per richiedere un controllo di verifica. Il processo di verifica cerca il record TXT aggiunto al passaggio precedente. Se trova il record TXT corretto, il dominio è verificato.  
...
2. Nella **colonna** Azione del  dominio che si sta verificando nella pagina Domini selezionare **Avvia configurazione.** 
3. Nella pagina **Confermare di essere** proprietari del dominio selezionare **Fatto,** verificare ora e quindi nella finestra di dialogo di conferma selezionare **Fine.** 
