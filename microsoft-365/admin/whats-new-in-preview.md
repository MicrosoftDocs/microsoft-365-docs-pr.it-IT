...
Il Centro messaggi è stato rinnovato per facilitare l'individuazione dei messaggi pertinenti e ha aggiunto un'esperienza di lettura più flessibile. È stata aggiunta una nuova colonna **Servizio** che consente di analizzare il servizio a cui si applica un messaggio e filtrare i messaggi in base al servizio e ad altri metadati. È possibile contrassegnare un messaggio come preferito per il follow-up, scegliere le colonne da visualizzare nell'elenco dei messaggi e spostarsi tra i messaggi con i pulsanti Indietro e Avanti. Abbiamo anche migliorato il processo per rendere più semplice fornire feedback sui post del Centro messaggi.
...
Con l'aumento dell'utilizzo di Teams, sono state aggiunte alcune funzionalità che consentono di gestirle.
...
- Dettagli utente con metadati dell'organizzazione nelle categorie Esperienze dei dipendenti
...
Un paio di modifiche nell'area dei gruppi di questo mese:
