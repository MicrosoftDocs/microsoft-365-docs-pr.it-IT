...
1.  Connettere PowerShell all'organizzazione di Exchange Online. Per istruzioni dettagliate, vedere [Connettersi a Exchange Online PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-powershell).
...
3. Eseguire i due comandi seguenti per aggiungere le altre proprietà dal file CSV ai contatti esterni creati nel passaggio 2.
    ...
    > [!NOTE]
    > Il  _parametro Manager_ potrebbe essere problematico. Se la cella è vuota nel file CSV, verrà visualizzato un errore e nessuna delle informazioni sulle proprietà verrà aggiunta al contatto. Se non è necessario specificare un manager, è sufficiente eliminare il comando  ` -Manager $_.Manager ` PowerShell precedente. 
