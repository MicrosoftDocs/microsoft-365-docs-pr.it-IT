...
### <a name="microsoft-defender-for-endpoint-preview"></a>Microsoft Defender per endpoint (anteprima)

[Microsoft Defender for endpoint](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/microsoft-defender-advanced-threat-protection) è una piattaforma di sicurezza per endpoint Enterprise progettata per aiutare le reti aziendali a impedire, rilevare, esaminare e rispondere alle minacce avanzate. Per una migliore visibilità della violazione della sicurezza nell'organizzazione, è possibile importare e filtrare i difensori per gli avvisi endpoint per le attività utilizzate nei criteri creati da modelli di criteri di violazione della sicurezza di gestione dei rischi Insider.

A seconda dei tipi di segnali che sono interessati, è possibile scegliere di importare avvisi per la gestione dei rischi insider in base al difensore dello stato di valutazione degli avvisi di endpoint. È possibile definire uno o più dei seguenti stati di valutazione degli avvisi nelle impostazioni globali da importare:
...
Gli avvisi provenienti da Defender per endpoint vengono importati ogni giorno. A seconda dello stato di valutazione scelto, è possibile che vengano visualizzate più attività utente per lo stesso avviso delle modifiche allo stato del triage in Defender for endpoint.

Ad esempio, se si seleziona *nuovo*, *in corso* e *risolto* per questa impostazione, quando viene generato un avviso di Microsoft Defender per endpoint e lo stato è *nuovo*, viene importata un'attività di avviso iniziale per l'utente in Risk Insider. Quando il difensore dello stato di valutazione dell'endpoint cambia *in corso*, viene importata una seconda attività per l'avviso per l'utente in Risk Insider. Quando viene impostato l'ultimo difensore dello stato di valutazione dell'endpoint *risolto* , viene importata una terza attività per questo avviso per l'utente in rischio Insider. Questa funzionalità consente agli investigatori di seguire la progressione del difensore per gli avvisi endpoint e scegliere il livello di visibilità richiesto dall'indagine.
