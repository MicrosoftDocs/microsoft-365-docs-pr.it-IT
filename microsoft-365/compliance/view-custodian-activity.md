---
title: Visualizzare l'attività di controllo del responsabile
f1.keywords:
- NOCSH
ms.author: markjjo
author: markjjo
...
- MET150
ms.assetid: ''
description: Utilizzare lo strumento Advanced eDiscovery Custodian Management per accedere facilmente all'attività e cercare i responsabile all'interno del caso.
...
---
# <a name="view-custodian-audit-activity"></a>Visualizzare l'attività di controllo del responsabile

Se è necessario verificare se un utente ha visualizzato un documento specifico o ha eliminato un elemento dalla cassetta postale, Advanced eDiscovery è ora integrato con lo strumento di ricerca dei log di controllo esistente nel Centro sicurezza & conformità. Utilizzando questa esperienza incorporata, è possibile utilizzare lo strumento Advanced eDiscovery Custodian Management per facilitare l'indagine accedendo facilmente all'attività e cercando i responsabile all'interno del caso.

## <a name="get-permissions"></a>Ottenere le autorizzazioni

È necessario avere il ruolo relativo ai log di controllo di sola lettura o ai log di controllo in Exchange Online per poter eseguire ricerche nel log di controllo. Per impostazione predefinita, questi ruoli sono assegnati ai gruppi di ruoli Gestione conformità e Gestione organizzazione nella pagina Autorizzazioni nell'Interfaccia di amministrazione di Exchange. Per offrire a un utente la possibilità di eseguire ricerche nel log di controllo di Advanced eDiscovery con il livello minimo di privilegi, è possibile creare un gruppo di ruoli personalizzato in Exchange Online, aggiungere il ruolo registri di controllo di View-Only o registri di controllo e quindi aggiungere l'utente come membro del nuovo gruppo di ruoli. Per altre informazioni, vedere Gestire i gruppi di ruoli in Exchange Online.
...
3. Nella pagina **Custodians** seleziona un responsabile nell'elenco e quindi fai clic su **View custodian activity** on the flyout page.

    Viene visualizzata la pagina di ricerca Delle attività del responsabile. Si noti che il responsabile selezionato nel passaggio precedente viene visualizzato nella casella di **riepilogo** a discesa Responsabile. È possibile selezionare diversi responsabile nella casella di riepilogo a discesa, ma è possibile cercare solo le attività per un responsabile alla volta.

    ![Pagina di ricerca delle attività del responsabile](../media/AeDCustodianActivities1.png)
...
4. Configurare i criteri di ricerca seguenti: 
   ...
   1. **Data di inizio e Data di fine:** selezionare una data e un intervallo di tempo per visualizzare gli eventi che si sono verificati all'interno di tale periodo. Gli ultimi sette giorni sono selezionati per impostazione predefinita. La data e l'ora sono specificate in formato UTC (Coordinated Universal Time). L'intervallo di date massimo che è possibile specificare è un anno.
      
   1. **Responsabile:** fare clic in questa casella e quindi selezionare un responsabile specifico per cui visualizzare i risultati della ricerca. I record di controllo per l'attività selezionata eseguita dagli utenti selezionati in questa casella vengono visualizzati nell'elenco dei risultati.
      
5. Fare clic su ![Pulsante Cerca](../media/SearchButton.PNG)  per eseguire la ricerca utilizzando i criteri di ricerca. I risultati della ricerca vengono caricati e dopo qualche minuto vengono visualizzati in Risultati nella pagina di ricerca Attività del responsabile. 
...
- **Dettagli:** dettagli aggiuntivi su un'attività. Anche in questo caso, non tutte le attività avranno un valore.
...
5. Per cancellare un filtro, fare clic sulla **X** nella casella del filtro o semplicemente su **Nascondi filtro.**

## <a name="export-the-search-results-to-a-file"></a>Esportare i risultati della ricerca in un file

È possibile esportare i risultati di una ricerca nel log di controllo in un file con valori delimitati da virgole (CSV) nel computer locale. È possibile aprire questo file in Microsoft Excel e utilizzare caratteristiche quali la ricerca, l'ordinamento, il filtro e la suddivisione di una singola colonna (che contiene celle multivalore) in più colonne.
...
3. Dopo aver selezionato un'opzione di esportazione, viene visualizzato un messaggio nella parte inferiore della finestra in cui viene richiesto di aprire il file CSV, salvarlo nella cartella Download o salvarlo in una cartella specifica

Per ulteriori informazioni sulla visualizzazione, il filtro o l'esportazione dei risultati della ricerca nei log di controllo, vedere Eseguire una ricerca nel log di controllo nel Centro [sicurezza & conformità.](search-the-audit-log-in-security-and-compliance.md)
