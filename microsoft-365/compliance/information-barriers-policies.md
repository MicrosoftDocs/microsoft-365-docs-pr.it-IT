...
description: Informazioni su come definire i criteri per le barriere alle informazioni in Microsoft Teams.
ms.author: robmazz
author: robmazz
manager: laurawi
audience: ITPro
...
- Gli attributi dell'account utente sono definiti in Azure Active Directory (o Exchange Online). Questi attributi possono includere reparto, posizione, posizione, nome del team e altri dettagli del profilo professionale. 
...
- I criteri di barriera delle informazioni determinano i limiti o le restrizioni di comunicazione. Quando si definiscono i criteri di barriera delle informazioni, è possibile scegliere tra due tipi di criteri:
...
- L'applicazione dei criteri viene eseguita dopo aver definito tutti i criteri di barriera delle informazioni e si è pronti ad applicarli nell'organizzazione.

## <a name="the-work-flow-at-a-glance"></a>Flusso di lavoro in breve

| Fase | Cosa è coinvolto |
|:--------|:------------------|
| [Verificare che i prerequisiti siano soddisfatti](#prerequisites) | - Verificare di disporre delle [licenze e delle autorizzazioni necessarie](information-barriers.md#required-licenses-and-permissions)<br/>- Verificare che la directory includa dati per la segmentazione degli utenti<br/>- Abilitare la ricerca di directory con ambito per Microsoft Teams<br/>- Verificare che la registrazione di controllo sia attivata<br/>- Assicurarsi che non siano presenti criteri della rubrica di Exchange<br/>- Usare PowerShell (vengono forniti esempi)<br/>- Fornire il consenso dell'amministratore per Microsoft Teams (sono inclusi i passaggi) |
...
- Consenso dell'amministratore per le barriere di informazioni in Microsoft Teams- Quando i criteri IB sono in atto, possono rimuovere gli utenti di conformità non IB dai gruppi (ad esempio, i canali di Teams, che si basano sui gruppi). Questa configurazione garantisce che l'organizzazione rimanga conforme ai criteri e alle normative. Utilizzare la procedura seguente per consentire ai criteri di barriera delle informazioni di funzionare come previsto in Microsoft Teams.

   1. Prerequisiti: installare Azure PowerShell da [Installare Azure PowerShell.](https://docs.microsoft.com/powershell/azure/install-az-ps)
...
| Criteri di barriera delle informazioni | Utilizzare il cmdlet **Get-InformationBarrierPolicy.** <p> Sintassi: `Get-InformationBarrierPolicy` <p> Questo cmdlet visualizza un elenco dei criteri di barriera delle informazioni definiti e il relativo stato. |
...
Per vedere come un'organizzazione potrebbe affrontare la definizione di segmenti e criteri, prendi in considerazione l'esempio seguente.

### <a name="contosos-departments-and-plan"></a>Reparto e piano di Contoso

Contoso ha cinque reparti: HR, Sales, Marketing, Research e Manufacturing. Per rimanere conformi alle normative del settore, gli utenti di alcuni reparti non devono comunicare con altri reparti, come indicato nella tabella seguente:
...
|:---------|:--------------------|
| **Criterio 1: Impedire alle vendite di comunicare con La ricerca** | `New-InformationBarrierPolicy -Name "Sales-Research" -AssignedSegment "Sales" -SegmentsBlocked "Research" -State Inactive` <p> In questo esempio, il criterio di barriera delle informazioni è *denominato Sales-Research.* Quando questo criterio è attivo e applicato, consente di impedire agli utenti che si trova nel segmento Vendite di comunicare con gli utenti nel segmento Ricerca. Questo criterio è un criterio unidiredire due. non impedirà a Research di comunicare con Sales. Per questo, è necessario il criterio 2. |
| **Criterio 2: Impedire alla ricerca di comunicare con Sales** | `New-InformationBarrierPolicy -Name "Research-Sales" -AssignedSegment "Research" -SegmentsBlocked "Sales" -State Inactive` <p> In questo esempio, il criterio di barriera delle informazioni è *denominato Research-Sales.* Quando questo criterio è attivo e applicato, impedirà agli utenti del segmento Ricerche di comunicare con gli utenti nel segmento Vendite. |
| **Criterio 3: consentire alla produzione di comunicare solo con hr e marketing** | `New-InformationBarrierPolicy -Name "Manufacturing-HRMarketing" -AssignedSegment "Manufacturing" -SegmentsAllowed "HR","Marketing","Manufacturing" -State Inactive` <p> In questo caso, il criterio di barriera delle informazioni è *denominato Manufacturing-HRMarketing.* Quando questo criterio è attivo e applicato, Manufacturing può comunicare solo con HR e Marketing. Le risorse umane e il marketing non sono limitati a comunicare con altri segmenti. |
