...
In genere, le nuove funzionalità OME sono abilitate automaticamente per l'organizzazione. Per ulteriori informazioni sulle nuove funzionalità OME all'interno dell'organizzazione, vedere [configurare le nuove funzionalità di crittografia dei messaggi di Office 365](set-up-new-message-encryption-capabilities.md).

Se è stata abilitata Azure Information Protection, la versione legacy di OME è abilitata automaticamente per l'organizzazione. In passato, la funzionalità OME Legacy ha funzionato anche se la protezione delle informazioni di Azure non è stata abilitata. Questo non è più il caso.
