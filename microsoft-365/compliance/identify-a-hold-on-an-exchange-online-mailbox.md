...
Nella tabella seguente viene descritto come identificare i diversi tipi di blocchi in base ai valori della proprietà *InPlaceHolds* quando si esegue il cmdlet **Get-Mailbox.**
...
|Criteri di conservazione di Microsoft 365 applicati in modo specifico alla cassetta postale     |    `mbxcdbbb86ce60342489bff371876e7f224:1` <br/> oppure <br/> `skp127d7cf1076947929bf136b7a2a8c36f:3`     |     La proprietà InPlaceHolds contiene i GUID di qualsiasi criterio di conservazione percorso specifico applicato alla cassetta postale. È possibile identificare i criteri di conservazione perché il GUID inizia con `mbx` il prefisso o il `skp` prefisso. Il `skp` prefisso indica che il criterio di conservazione viene applicato alle conversazioni di Skype for Business nella cassetta postale dell'utente.    |
...
Se la *proprietà InPlaceHolds* è vuota quando si esegue il cmdlet **Get-Mailbox,** alla cassetta postale potrebbero essere ancora applicati uno o più criteri di conservazione di Microsoft 365 a livello di organizzazione. Eseguire il seguente comando in PowerShell di Exchange Online per ottenere un elenco dei GUID per i criteri di conservazione di Microsoft 365 a livello di organizzazione.
...
Eseguire il seguente comando in & PowerShell per Centro sicurezza e conformità per identità dei criteri di conservazione di Microsoft 365 (posizione specifica o a livello di organizzazione) applicati alla cassetta postale. Usa il GUID (non incluso il prefisso mbx, skp o grp o il suffisso di azione) identificato nel passaggio 1.
...
- Eseguire il [comando Set-Mailbox \<hold GUID without prefix or suffix> -ExcludeFromOrgHolds](https://docs.microsoft.com/powershell/module/exchange/set-mailbox) in PowerShell di Exchange Online per escludere una cassetta postale inattiva dai criteri di conservazione di Microsoft 365 a livello di organizzazione.
