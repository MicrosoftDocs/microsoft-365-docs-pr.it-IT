...
In questo argomento vengono descritti i passaggi da eseguire per attivare e configurare Azure Rights Management (RMS), parte di Azure Information Protection, per l'utilizzo con la versione precedente di Office 365 Message Encryption (OME).
...
Se l'organizzazione non è stata ancora spostata nelle nuove funzionalità OME, ma si è già distribuito OME, le informazioni contenute in questo articolo si applicano all'organizzazione. Microsoft consiglia di pianificare il passaggio alle nuove funzionalità OME non appena è ragionevole per l'organizzazione. Per istruzioni, vedere [Set up new Office 365 Message Encryption capabilities](set-up-new-message-encryption-capabilities.md). Per ulteriori informazioni sul funzionamento delle nuove funzionalità, vedere Office 365 Message Encryption [.](ome.md) Il resto di questo articolo fa riferimento al comportamento OME prima del rilascio delle nuove funzionalità OME.
...
- Se non si è certi di cosa include l'abbonamento, vedere le descrizioni dei servizi di Exchange Online per Criteri messaggio, Ripristino [e conformità.](/office365/servicedescriptions/exchange-online-service-description/message-policy-and-compliance)
...
6. Eseguire il cmdlet [Set-IRMConfiguration](/powershell/module/exchange/set-irmconfiguration) come indicato di seguito per disabilitare la disponibilità dei modelli di Azure Rights Management Outlook sul Web e Outlook: 

   ```powershell
   Set-IRMConfiguration -ClientAccessServerEnabled $false
   ```

7. Eseguire il cmdlet [Set-IRMConfiguration](/powershell/module/exchange/set-irmconfiguration) come indicato di seguito per abilitare Azure Rights Management per l'organizzazione di posta elettronica basata su cloud e configurarlo per l'utilizzo di Azure Rights Management per Office 365 Message Encryption:
...
È possibile continuare a utilizzare le regole del Office 365 Message Encryption di posta elettronica esistenti con Active Directory Rights Management, ma non è possibile configurare o utilizzare le nuove funzionalità OME. È invece necessario eseguire la migrazione ad Azure Information Protection. Per informazioni sulla migrazione e su cosa significa per l'organizzazione, vedere [Migrating from AD RMS to Azure Information Protection.](/information-protection/deploy-use/prepare-environment-adrms)
  
## <a name="next-steps"></a>Passaggi successivi
<a name="importTPDs"> </a>

Dopo aver completato la configurazione di Azure Rights Management, se si desidera abilitare le nuove funzionalità OME, vedere Configurare nuove funzionalità di Office 365 Message Encryption integrate in [Azure Information Protection.](./set-up-new-message-encryption-capabilities.md)
