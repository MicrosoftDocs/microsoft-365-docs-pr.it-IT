...
I criteri di conservazione consentono di eseguire questa operazione in modo molto efficace assegnando le stesse impostazioni di conservazione per il contenuto in base alla posizione, al sito o alla cassetta postale. Se non si è certi di usare i criteri di conservazione o un'etichetta di conservazione, vedere [Criteri di conservazione e etichette di conservazione](retention.md#retention-policies-and-retention-labels).
...
L'amministratore globale dell'organizzazione dispone delle autorizzazioni complete per creare e modificare i criteri di conservazione. Se non si esegue l'accesso come amministratore globale, vedere [Autorizzazioni necessarie per creare e gestire i criteri di conservazione e le etichette di conservazione](get-started-with-retention.md#permissions-required-to-create-and-manage-retention-policies-and-retention-labels).
...
Se si selezionano le posizioni di Teams o Yammer quando si creano i criteri di conservazione, le altre posizioni vengono escluse automaticamente. Di conseguenza, le istruzioni da seguire variano in base alla necessità di includere le posizioni di Teams o di Yammer:
...
3. Nella pagina **Scegli posizioni a cui applicare il criterio**, selezionare una o entrambe le posizioni di Teams: **Messaggi del canale di Teams** e **Chat di Teams**.
   ...
   Per i **messaggi del canale di Teams**, sono inclusi i messaggi provenienti da canali standard, ma non da [canali privati](https://docs.microsoft.com/microsoftteams/private-channels). Attualmente, i canali privati non sono supportati dai criteri di conservazione.
...
4. Nella pagina **Decidere se si vuole conservare il contenuto, eliminarlo e entrambi** della procedura guidata specificare le opzioni di configurazione per la conservazione e l'eliminazione del contenuto.
   ...
   È possibile creare un criterio che si limita a conservare il contenuto senza eliminarlo, lo conserva e quindi lo elimina dopo un periodo di tempo specificato oppure semplicemente elimina il contenuto dopo un periodo di tempo specificato. Per ulteriori informazioni vedere [Impostazioni per conservare e eliminare contenuto](#settings-for-retaining-and-deleting-content) su questa pagina.
...
Teams offre molto di più oltre alle chat e i messaggi di canale. Se sono presenti team creati da un gruppo di Microsoft 365 (in precedenza gruppo di Office 365), è consigliabile configurare anche un criterio di conservazione che includa tale gruppo di Microsoft 365 usando il percorso **Gruppi di Microsoft 365**. Questo criteri di conservazione si applica a contenuti situati nella casella di posta, nel sito e nei file del gruppo.
...
È possibile che un criterio di conservazione applicato ai gruppi di Microsoft 365, ai siti di SharePoint o agli account di OneDrive sia in grado di eliminare un file a cui viene fatto riferimento in una chat di Teams o un messaggio di canale prima che quei messaggi vengano eliminati. In questo scenario, il file viene ancora visualizzato nel messaggio di Teams, ma quando gli utenti selezionano il file, viene visualizzato un errore di tipo "file non trovato". Questo comportamento non è specifico dei criteri di conservazione e può verificarsi anche se un utente elimina manualmente un file da SharePoint o da OneDrive.

### <a name="retention-policy-for-yammer-locations"></a>Criterio di conservazione per posizioni di Yammer

> [!NOTE]
> I criteri di conservazione per Yammer sono implementati in anteprima. Se i nuovi percorsi di Yammer non sono ancora visualizzati, riprovare tra qualche giorno.
>
...
4. Nella pagina **Scegliere posizioni** selezionare **Consenti la scelta di posizioni specifiche**. Quindi, attivare una o entrambe le posizioni di Yammer: **Messaggi della community di Yammer** e **Messaggi privati di Yammer**.
...
Yammer offre molto di più oltre ai messaggi della community e i messaggi privati. Per conservare e cancellare i messaggi di posta elettronica della rete Yammer, configurare un altro criterio di conservazione che includa tutti i gruppi di Microsoft 365 usati per Yammer usando la posizione **Gruppi di Microsoft 365**. 
...
È possibile che un criterio di conservazione applicato ai siti di SharePoint o agli account di OneDrive sia in grado di eliminare un file a cui viene fatto riferimento in un messaggio di Yammer prima che quei messaggi vengano eliminati. In questo scenario, il file viene ancora visualizzato nel messaggio di Yammer, ma quando gli utenti selezionano il file, viene visualizzato un errore di tipo "file non trovato". Questo comportamento non è specifico dei criteri di conservazione e può verificarsi anche se un utente elimina manualmente un file da SharePoint o da OneDrive.
...
3. Nella pagina **Scegliere posizioni** attivare o disattivare le posizioni desiderate, tranne quelle di Teams. Per ogni posizione è possibile tenerla come impostazione predefinita per [applicare il criterio all’intera posizione](#a-policy-that-applies-to-entire-locations), o [specificare includere e escludere](#a-policy-with-specific-inclusions-or-exclusions).
...
Si noti che anche se un gruppo di Microsoft 365 ha una cassetta postale di **Exchange** un criterio di conservazione che include l'intero percorso di Exchange non includerà il contenuto nelle cassette postali del gruppo di **Microsoft 365. Per conservare il contenuto in queste cassette postali, selezionare posizione di** Gruppi di Microsoft 365.
...
Selezionando la posizione **siti di SharePoint**, il criterio di conservazione può conservare ed eliminare documenti all'interno di siti di comunicazione di SharePoint, siti del team non collegati a gruppi di Microsoft 365 e siti classici. I siti di Teams collegati dai gruppi di Microsoft 365 non sono supportati con questa opzione. Quindi, usarela posizione **Gruppi Microsoft 365** applicata ai contenuti nella casella postale, sito e file del gruppo.
...
Quando si seleziona **Scegliere utente**, si possono includere rapidamente tutti gli utenti selezionando la casella **Seleziona tutto**. È necessario considerare, però, che ciascun utente viene conteggiato come specifica inclusione nei criteri. Pertanto, se si includono 1000 utenti selezionando la casella **Seleziona tutto**, equivarrà a selezionare manualmente i 1000 utenti da includere, che è il numero massimo supportato da Skype for Business.
...
- Conserva solo
    ...
    Per questa configurazione, scegliere **Conserva elementi per un periodo specifico** e **Al termine del periodo di conservazione: non fare nulla**. Oppure, selezionare **Conserva elementi per sempre**.
