...
Ecco un elenco dei processi (che in genere sono processi a esecuzione lunga) che vengono monitorati nella scheda **Processi** di un caso in Advanced eDiscovery. Questi processi vengono attivati dalle azioni dell'utente durante l'utilizzo e la gestione dei casi.
...
|Aggiunta di dati non Microsoft 365 a un set di recensioni | Un utente carica dati non Microsoft 365 in un set di recensioni. Anche i dati vengono indicizzati durante questo processo. Ad esempio, i file di un file server locale o di un computer client vengono caricati in un set di revisione. Per ulteriori informazioni, vedere [Load non-Microsoft 365 data into a review set](load-non-office-365-data-into-a-review-set.md).| 
...
|Confronto dei set di carico | Un utente esamina le differenze tra diversi set di carico in un set di revisione. Un set caricato è un'istanza dell'aggiunta di dati a un insieme da rivedere. Se ad esempio si aggiungono i risultati di due ricerche diverse allo stesso set di revisioni, ognuna rappresenterebbe un set di carico. |
...
|Esecuzione dell'analisi | Un utente analizza i dati in un set di revisione eseguendo strumenti di analisi Advanced eDiscovery, ad esempio il rilevamento quasi duplicato, l'analisi del threading della posta elettronica e l'analisi dei temi. Per ulteriori informazioni, vedere [Analyze data in a review set.](analyzing-data-in-review-set.md) | 
