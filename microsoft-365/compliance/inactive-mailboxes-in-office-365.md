...
Oltre al blocco per controversia legale, l'uso della nuova funzionalità dei criteri di conservazione di Microsoft 365 nel Centro sicurezza & conformità è un altro modo per rendere inattiva una cassetta postale. To use a retention policy to make an inactive mailbox:

- Deve essere configurato per conservare o conservare ed eliminare il contenuto. Se un criterio di conservazione è configurato per eliminare solo il contenuto, una cassetta postale a cui viene applicato il criterio non diventerà inattiva quando la cassetta postale viene eliminata.
...
- È possibile identificare rapidamente il criterio di conservazione assegnato alle cassette postali inattive nell'organizzazione. In questo modo è più semplice modificare le impostazioni di conservazione (o eliminazione), se necessario. Sarà inoltre più semplice eliminare definitivamente una cassetta postale inattiva perché è possibile rimuoverla dal criterio utilizzando il Centro sicurezza & conformità. In caso contrario, è necessario utilizzare PowerShell di Exchange Online per rimuovere un blocco per controversia legale da una cassetta postale inattiva o utilizzare PowerShell per Centro sicurezza & conformità per escludere una cassetta postale inattiva dai criteri di conservazione di Microsoft 365 a livello di organizzazione.
...
Per rendere inattiva una cassetta postale, è necessario assegnare una licenza di Exchange Online Piano 2 (o una licenza di Exchange Online Piano 1 con una licenza del componente aggiuntivo Archiviazione Exchange Online) in modo che un blocco per controversia legale o i criteri di conservazione di Microsoft 365 possano essere applicati alla cassetta postale prima che venga eliminata. Dopo l'eliminazione della cassetta postale, tutte le licenze di Exchange Online ad essa associate saranno disponibili per l'assegnazione a un nuovo utente.
...
- **Recuperare una cassetta postale inattiva.** Se un ex dipendente (o un dipendente in congedo) torna nell'organizzazione o se un nuovo dipendente viene assunto per assumersi le responsabilità di lavoro dell'ex dipendente, è possibile recuperare il contenuto della cassetta postale inattiva. Quando si recupera una cassetta postale inattiva, la cassetta postale viene convertita in una nuova cassetta postale, il contenuto e la struttura di cartelle della cassetta postale inattiva vengono mantenuti e la cassetta postale viene collegata a un nuovo account utente. Dopo essere stata recuperata, la cassetta postale inattiva non esiste più. Per procedure dettagliate e informazioni su cosa accade quando si recupera una cassetta postale inattiva, vedere Recuperare una cassetta postale [inattiva.](recover-an-inactive-mailbox.md)
  ...
  > [!NOTE]
  > Se si recupera una cassetta postale inattiva assegnata a un criterio di conservazione con protezione dell'archiviazione (denominata criterio di conservazione *bloccato),* la cassetta postale recuperata viene assegnata allo stesso criterio di conservazione bloccato. Se si recupera una cassetta postale inattiva assegnata a un criterio di conservazione senza la protezione dell'archiviazione, la cassetta postale recuperata viene rimossa dal criterio di conservazione sbloccato. Tuttavia, il blocco per controversia legale è abilitato sulla cassetta postale recuperata per impedire l'eliminazione del contenuto delle cassette postali in base a criteri di conservazione a livello di organizzazione che eliminano contenuti precedenti a una data di validità specifica.
