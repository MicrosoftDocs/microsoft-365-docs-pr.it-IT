---
title: Elenco dei modelli di Microsoft Compliance Manager
f1.keywords:
- NOCSH
ms.author: chvukosw
author: chvukosw
manager: laurawi
audience: Admin
ms.topic: article
ms.service: O365-seccomp
localization_priority: Normal
ms.collection: M365-security-compliance
search.appverid:
- MOE150
- MET150
description: Microsoft Compliance Manager fornisce modelli per la creazione di valutazioni in linea con le normative, gli standard e le leggi nazionali, regionali e del settore.
ms.openlocfilehash: 73777746606c20d43cdf330b04e7301b5c512bcc
ms.sourcegitcommit: 070724118be25cd83418d2a56863da95582dae65
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 03/03/2021
ms.locfileid: "50406748"
---
# <a name="compliance-manager-templates-list"></a>Elenco dei modelli di Compliance Manager

**In questo articolo:** Visualizzare l'elenco completo **dei modelli** disponibili per la creazione di valutazioni in Compliance Manager.

> [!IMPORTANT]
> I modelli di valutazione disponibili per l'organizzazione dipendono dal contratto di licenza. [Esaminare i dettagli.](https://go.microsoft.com/fwlink/?linkid=2132371)

## <a name="overview"></a>Panoramica

[Microsoft Compliance Manager](compliance-manager.md) fornisce un set completo di modelli per la creazione di valutazioni. Questi modelli possono aiutare l'organizzazione a rispettare i requisiti nazionali, regionali e specifici del settore che regolano la raccolta e l'uso dei dati.

I modelli vengono aggiunti a Compliance Manager durante l'applicazione di nuove leggi e normative. Compliance Manager aggiorna i modelli quando cambiano le leggi o le normative sottostanti. Ulteriori informazioni su come [rivedere e accettare gli aggiornamenti.](compliance-manager-assessments.md#accepting-updates-to-assessments)

Visualizzare indicazioni dettagliate [sull'utilizzo dei modelli](compliance-manager-templates.md) per creare le valutazioni.

## <a name="list-of-templates-and-where-to-find-them"></a>Elenco di modelli e dove trovarli

Di seguito è riportato l'elenco completo dei modelli in Compliance Manager. Ogni pagina dei dettagli del modello fornisce informazioni sul regolamento o sullo standard a cui si applica. I collegamenti nei nomi dei modelli riportati di seguito illustrano la documentazione correlata relativa a tale standard, normativa o legge.

**Dove trovare i modelli**

In Compliance Manager passare alla pagina **modelli di** valutazione. Verrà visualizzato un elenco di tutti i modelli disponibili per l'organizzazione. È importante comprendere la distinzione tra modelli inclusi e premium:

- **I modelli inclusi** sono modelli inclusi nel contratto di licenza dell'organizzazione.
 - **I modelli** Premium visualizzano modelli aggiuntivi che l'organizzazione può scegliere di ottenere (fare riferimento alle [condizioni del servizio).](https://go.microsoft.com/fwlink/?linkid=2132371)

Altre informazioni [su come visualizzare e gestire i modelli.](compliance-manager-templates.md#viewing-and-managing-templates-from-the-assessment-templates-page)

> [!NOTE]
> Per [ulteriori informazioni sui](compliance-manager-templates.md#template-types-included-and-premium-active-and-inactive) modelli inclusi e premium e per informazioni sui modelli disponibili per l'organizzazione, vedere Tipi di modello.

## <a name="included-templates"></a>Modelli inclusi

- [Microsoft Data Protection Baseline](compliance-manager-assessments.md#data-protection-baseline-default-assessment)
- [GDPR](https://docs.microsoft.com/compliance/regulatory/gdpr) dell'Unione Europea (Microsoft 365, Office 365, Intune)
- [ISO 27001:2013](https://docs.microsoft.com/compliance/regulatory/offering-iso-27001)
- NIST 800-53 Rev.4

> [!NOTE]
> Per i clienti della US Government Community (GCC) Moderate e GCC High, sono inclusi i modelli CmMC (Cybersecurity Maturity Model Certification) da 1 a 5, oltre ai modelli sopra elencati.

## <a name="premium-templates"></a>Modelli Premium

- AICPA/CICA Generally Accepted Privacy Principles (GAPP) (Microsoft 365)
- Alabama - Criterio 621: Notifica di violazione dei dati (Microsoft 365)
- Alaska - Capitolo 48 - Legge sulla protezione delle informazioni personali (Microsoft 365)
- Albania - Legge sulla protezione dei dati personali n. 9887
- Antigua e Barbuda-Data Protection Act /2013 (Microsoft 365)
- Appendice III - Numero circolare OMB A-130 - Sicurezza delle risorse di informazioni automatizzate federali
- [Argentina - Legge sulla protezione dei dati personali 25.326](https://docs.microsoft.com/compliance/regulatory/offering-pdpa-argentina) (Microsoft 365)
- Arkansas - Personal Information Protection Act (Microsoft 365)
- Asia Pacific Economic Cooperation (APEC) Privacy Framework
- Australia - ASD Essential 8 (Microsoft 365)
- Australia - National Archives Act
- Australia - Public Records Office Recordkeeping Standards (Microsoft 365)
- Australia - Spam Act 2003 (Microsoft 365)
- Australia Privacy (Credit Reporting) Code 2014 (versione 2.1)
- Australian Energy Sector Cyber Security Framework (AESCSF) (Microsoft 365)
- [Australian Information Security Registered Assessor Program (IRAP) versione 2](https://docs.microsoft.com/compliance/regulatory/offering-ccsl-irap-australia) (Microsoft 365)
- [Australian Prudential Regulation Authority CPS](https://docs.microsoft.com/compliance/regulatory/offering-apra-australia) (Microsoft 365)
- Legge sulle telecomunicazioni 2003 (Microsoft 365)
- Bahamas - Data Protection Act (Microsoft 365)
- Barbados - Data Protection Bill 2019 (Microsoft 365)
- Legge bielorussa sulle informazioni, l'informatizzazione e la protezione delle informazioni (Microsoft 365)
- [Belgio NBB dicembre 2015](https://docs.microsoft.com/compliance/regulatory/offering-nbb-fsma-belgium) (Microsoft 365)
- Bermuda - Electronic Transaction Act (Microsoft 365)
- Legge sulla protezione dei dati personali in Bosnia ed Erzegovina
- Brasile - Legge sul codice di protezione dei consumatori n. 8078 (Office 365)
- Brasile - General Data Protection Law (LGPD) (Microsoft 365)
- Legge bulgara per la protezione dei dati personali 2002 (Microsoft 365)
- California - Sezione del codice civile 1798
- California - Database Breach Act (California SB 1386)
- California - Education Code-EDC, Title 3, Division 14, Part 65, Chapter 2.5 - Social Media Privacy
- California - Privacy delle informazioni SB-327: dispositivi connessi (Microsoft 365)
- California Consumer Credit Reporting Agencies Act (Microsoft 365)
- [California Consumer Privacy Act (CCPA)](https://docs.microsoft.com/compliance/regulatory/offering-ccpa) (Microsoft 365)
- Canada - Normative sulla violazione delle misure di sicurezza (Microsoft 365)
- Canada - British Columbia - Information Privacy & Security - FOIPPA (Microsoft 365)
- [Canada - Ufficio del sovrintendente degli istituti](https://docs.microsoft.com/compliance/regulatory/offering-osfi-canada) finanziari (Microsoft 365)
- Canada - Personal Health Information Protection Act (PHIPA) (Microsoft 365)
- Canada - Personal Information Protection and Electronic Documents Act (PIPEDA) (Microsoft 365)
- Canada - Protetto B
- Canada Cybersecure (Microsoft 365)
- CAN-SPAM Act (Microsoft 365)
- [CDSA Content Protection & Security Standard](https://docs.microsoft.com/compliance/regulatory/offering-cdsa) (Microsoft 365)
- [CFR - Codice delle normative federali Titolo 21](https://docs.microsoft.com/compliance/regulatory/offering-fda-cfr-title-21-part-11) (Microsoft 365)
- Standard anti-autoricilità per le strutture di evasore (CFATS) (Microsoft 365)
- Regola di protezione della privacy online per bambini (COPPA) (Microsoft 365)
- Cina - Specifica di sicurezza delle informazioni personali (Microsoft 365)
- [CIS Implementation Group 1, Group 2, Group 3](https://docs.microsoft.com/compliance/regulatory/offering-cis-benchmark)
- [Cloud Security Alliance (CSA) Cloud Controls Matrix (CCM)](https://docs.microsoft.com/compliance/regulatory/offering-csa-star-attestation)
- CMMC Livello 1, Livello 2, Livello 3, Livello 4, Livello 5 (Microsoft 365)
- COBIT 5 (Microsoft 365)
- Colombia - Decreto n. 1377/2013 (usato come legge 1581/2012 della Colombia)
- Colombia - Lettera circolare esterna 007 del 2018 (Microsoft 365)
- Colombia - Legge 1266/2008 - Habeas Data Act (Microsoft 365)
- Colombia - Legge 1581/2012 (Microsoft 365)
- Dichiarazione della Commissione e linee guida sulle divulgazioni di cybersecurity della società pubblica - Stati Uniti
- Computer Fraud and Abuse Act (CFAA) (Microsoft 365)
- Statuti generali connecticut - Disposizioni generali per gli appaltatori statali che ricevono informazioni riservate (Microsoft 365)
- Connecticut State Law - Violazione dei dati computerizzati di sicurezza contenenti informazioni personali (Microsoft 365)
- Consumer Personal Information Security Breach Notification Act (Microsoft 365)
- Criteri di sicurezza [CJIS (Criminal Criminal Information Services)](https://docs.microsoft.com/compliance/regulatory/offering-cjis) (Microsoft 365)
- Croazia - Legge sulla protezione dei dati personali (Microsoft 365)
- Legge sulla cybersecurity della Repubblica popolare cinese (Microsoft 365)
- Cipro - Trattamento della legge sui dati personali (Microsoft 365)
- Ceco - Atto n. 110/2019 Coll. sul trattamento dei dati personali - 2019 (Microsoft 365)
- Ceco - On Cyber Security and Change of Related Acts (Act on Cyber Security) - Act No. 181 (Microsoft 365)
- Delaware Computer Security Breaches - Commerce and Trade Subtitle II - 12B-100 to 12B-104
- Danimarca - Data Protection Act
- Danimarca - Ordine esecutiva sulle informazioni e il consenso richiesti in caso di archiviazione e accesso alle informazioni in End-User terminali
- [DFARS](https://docs.microsoft.com/compliance/regulatory/offering-dfars) (Microsoft 365)
- Direttiva 2013/40/UE dell'Europa e del Consiglio (Microsoft 365)
- Dubai - Regolamento sulla protezione dei dati sanitari (Microsoft 365)
- Regolamento sulla protezione dei consumatori (Autorità di regolamentazione delle telecomunicazioni) (Microsoft 365)
- Dubai ISR (Microsoft 365)
- Codice elettronico delle normative federali - Parte 748.0 e Appendice A (Microsoft 365)
- ENISA Information Assurance Framework
- Estonia - Personal Data Protection Act (Microsoft 365)
- Estonia - Sistema di misure di sicurezza per i sistemi in informazioni (Microsoft 365)
- UE - Direttiva ePrivacy 2002 58 EC (Microsoft 365)
- UE - EudraLex Volume 4 - Linee guida GMP, allegato 11
- Direttiva UE 2006/24/EC
- Regole sulla privacy FDIC (Microsoft 365)
- [Federal Financial Institutions Examination Council (FFIEC) Information Security Booklet](https://docs.microsoft.com/compliance/regulatory/offering-ffiec-us) (Microsoft 365, Intune)
- [Controlli fedramp ad alta sicurezza](https://docs.microsoft.com/compliance/regulatory/offering-fedramp) (Office 365)
- [FedRamp High Security Controls_NIST 800-53](https://docs.microsoft.com/compliance/regulatory/offering-fedramp) (Microsoft 365)
- [FedRAMP Moderato](https://docs.microsoft.com/compliance/regulatory/offering-fedramp)
- Finlandia - Data Protection Act
- Criteri finlandese per la valutazione della sicurezza delle informazioni dei servizi cloud (Microsoft 365)
- FINRA Cybersecurity Checklist
- Francia - Legge 78-17 del 6 gennaio 1978 su information technology, file di dati e libertà civile (Microsoft 365)
- Freedom of Information Act (FOIA) (Microsoft 365)
- Privacy FTC delle informazioni finanziarie degli utenti (Microsoft 365)
- Ghana Data Protection Act
- Principi di gestione dei record generalmente accettati (Microsoft 365)
- [Germania - Cloud Computing Compliance Controls Catalog (C5)](https://docs.microsoft.com/compliance/regulatory/offering-c5-germany) (Microsoft 365)
- Germania - Federal Data Protection Act (Microsoft 365)
- [Gramm-Leach-Bliley Act, Titolo V, Sottotitolo A, Privacy finanziaria](offering-glba.md) (Microsoft 365)
- Grecia - Legge 2472/1997 sulla protezione dei singoli utenti per quanto riguarda il trattamento dei dati personali (Microsoft 365)
- Hawaii - Violazione della sicurezza delle informazioni personali Capitolo 487N
- [HIPAA/HITECH](https://docs.microsoft.com/compliance/regulatory/offering-hipaa-hitech) (Microsoft 365, Intune)
- [HITRUST](https://docs.microsoft.com/compliance/regulatory/offering-hitrust) (Microsoft 365)
- Hong Kong - Ordinanza per i dati personali (Privacy) (Microsoft 365)
- India - IT Act del 2000 (Microsoft 365)
- India Information Technology (ragionevoli procedure e procedure di sicurezza e dati personali sensibili o informazioni)
- Indonesia - Legge 11/2008 (Microsoft 365)
- Indonesia - Peraturan Pemerintah No.82 Tahun 2012 - Government Regulation - Data Protection Regulation (Microsoft 365)
- IRAP v3
- [IRS-P1075](https://docs.microsoft.com/compliance/regulatory/offering-irs-1075) (Microsoft 365)
- IRS - Revenue Procedure 98-25 Automated Records
- ISO 15489 (Microsoft 365)
- [ISO 22301:2019](https://docs.microsoft.com/compliance/regulatory/offering-iso-22301) (Microsoft 365)
- ISO 27005:2018 (Microsoft 365)
- [ISO 27017:2015](https://docs.microsoft.com/compliance/regulatory/offering-iso-27017) (Microsoft 365)
- IsO 27799 Health informatics — Information security management in health using ISO/IEC 27002 (Microsoft 365)
- ISO 31000:2018 (Microsoft 365)
- IsO 80001-1 Applicazione di gestione dei rischi per reti IT che incorporano dispositivi medicali (Microsoft 365)
- [ISO/IEC 27018:2014](https://docs.microsoft.com/compliance/regulatory/offering-iso-27018)
- [ISO/IEC 27701:2019](https://docs.microsoft.com/compliance/regulatory/offering-iso-27701) (Microsoft 365)
- Israele - Normative sulla protezione della privacy (trasferimento dei dati ai database esestere) (Microsoft 365)
- ITU X.1052 Information Security Management Framework (Microsoft 365)
- Giappone - Act on Prohibition of Unauthorized Computer Access (Microsoft 365)
- Giappone - Modello comune di misure di sicurezza delle informazioni per agenzie governative e agenzie correlate (Microsoft 365)
- Giappone - Standard comuni per le misure di sicurezza delle informazioni per agenzie governative e agenzie correlate
- Japan Privacy Mark
- Japanese Act on the Protection of Personal Information (Law No. 57 del 2003) (Microsoft 365)
- Joint Commission AHO Information Management Standard (Microsoft 365)
- Kenya Data Protection Act (Microsoft 365)
- Corea - Legge sulla promozione dell'utilizzo e dei dati della rete di informazioni e comunicazioni - Protezione (Microsoft 365)
- Corea - Use and Protection of Credit Information Act (legge speciale) (Microsoft 365)
- Korea Personal Information Protection Act (Microsoft 365)
- Kuwait - CSF (Microsoft 365)
- Legge lussemburghese (Microsoft 365)
- Maine - Act to Protect the Privacy of Online Consumer Information
- Maine - Avviso di rischio per i dati personali (Microsoft 365)
- Malaysia - Personal Data Protection Act (PDPA) (Microsoft 365)
- Malaysia Risk Management in Technology (RMiT) (Microsoft 365)
- Massachusetts - 201 CMR 17.00: Standard per la protezione delle informazioni personali di - Residenti del Massachusettth (Microsoft 365)
- Mauritius Data Protection Act 2004 (Microsoft 365)
- Messico - Legge federale sulla protezione dei consumatori (Microsoft 365)
- Legge federale sulla protezione dei dati in Messico (Microsoft 365)
- Standard minimi di rischio accettabili per Exchange (MARS-E) 2.0 (Microsoft 365)
- Procedure consigliate per la sicurezza del contenuto di [Motion Picture Association (MPA)](https://docs.microsoft.com/compliance/regulatory/offering-mpaa) (Microsoft 365)
- Myanmar - Legge che protegge la privacy e la sicurezza dei cittadini
- NAIC - Standard per la protezione del regolamento MDL-673 del modello di informazioni dei clienti (Microsoft 365)
- Nepal - Diritto all'informazione
- [NERC CIP](https://docs.microsoft.com/compliance/regulatory/offering-nerc-cip) (Microsoft 365)
- Paesi Bassi - Legge sulla protezione dei dati personali / 1999 (Microsoft 365)
- Capitolo 603A di Vegas - Sicurezza e privacy delle informazioni personali (Microsoft 365)
- New York Privacy Act - DRAFT (Microsoft 365)
- Criteri di conservazione dei dati sanitari in Nuova Zelanda (Office 365)
- New Zealand Health Information Privacy Code 1994 (Microsoft 365)
- New Zealand Health Information Security Framework (HISF) -2015 (Microsoft 365)
- New Zealand Privacy Act 2020 (Microsoft 365)
- New Zealand Public Records Act (Microsoft 365)
- New Zealand Telecommunications Information Privacy Code 2003
- Regolamento sulla protezione dei dati della Nigeria (Microsoft 365)
- NIST 800-37 (Microsoft 365)
- NIST 800-53
- Linee guida per l'identità digitale NIST 800-63 (Microsoft 365)
- [NIST 800-171](https://docs.microsoft.com/compliance/regulatory/offering-nist-sp-800-171) (Microsoft 365)
- [NIST CSF](https://docs.microsoft.com/compliance/regulatory/offering-nist-csf) (Microsoft 365)
- NIST Privacy Framework
- Pubblicazione speciale NIST 800-128 (Microsoft 365)
- NIST Special Publication 1800-1 Securing Electronic Health Records on Mobile Devices (Microsoft 365)
- NIST Special Publication 1800-5 IT Asset Management
- Norvegia - Personal Data Act (Microsoft 365)
- NYDFS (Microsoft 365)
- Oman - Electronic Transactions Law (Microsoft 365)
- Controlli OWASP ProActive per sviluppatori 2018 v3.0 (Microsoft 365)
- Pakistan Electronic Data Protection Act 2005 - Draft (Microsoft 365)
- [PCI DSS v3.2.1](https://docs.microsoft.com/compliance/regulatory/offering-pci-dss) (Microsoft 365)
- Legge 29733 della legge 29733 sulla protezione della privacy dei dati
- Linee guida per la gestione della sicurezza delle informazioni BSP filippine (Microsoft 365)
- Philippines Data Privacy Act del 2012 (Microsoft 365)
- Privacy of Consumer Financial and Health Information Regulation, NAIC MDL-672, Q2 2017 (Microsoft 365)
- Portorico - Citizen Information on Data Banks Security Act (Microsoft 365)
- Criteri di sicurezza cloud del Qatar
- Criteri di esternalizzazione RBNZ BS11 (Microsoft 365)
- Legge della Repubblica di Moldova sulla protezione dei dati personali (Microsoft 365)
- [Reserve Bank of India Cyber Security Framework](https://docs.microsoft.com/compliance/regulatory/offering-rbi-irdai-india) (Microsoft 365)
- Romania - Legge sulla protezione dei dati 190/2018 (Microsoft 365)
- Russia - Legge federale 149-FZ su informazioni, tecnologie dell'informazione e sicurezza delle informazioni
- [Legge federale della Federazione russa relativa ai dati](https://docs.microsoft.com/compliance/regulatory/offering-russia-data-localization) personali (Microsoft 365)
- Saint Lucia Data Protection Act (Microsoft 365)
- [SEC 17-4(a)](https://docs.microsoft.com/compliance/regulatory/offering-sec-17a-4) (Microsoft 365)
- SIG (Microsoft 365)
- Singapore - Banking Act (Cap.19)
- Singapore - Cybersecurity 2018 (Microsoft 365)
- Singapore - Guida alla cyber security di IMDA IoT (Microsoft 365)
- Singapore - Monetary Authority of Singapore Technology Risk Management Framework (Microsoft 365)
- [Singapore - Multi-Tier Cloud Security (MTCS) Standard](https://docs.microsoft.com/compliance/regulatory/offering-mtcs-singapore) (Microsoft 365)
- Singapore - Report di controllo del provider di servizi esternamente (OSPAR) (Microsoft 365)
- Singapore - Personal Data Protection Act / 2012 (Microsoft 365)
- Singapore Spam Control Act (Microsoft 365)
- [SOC 1](https://docs.microsoft.com/compliance/regulatory/offering-soc) (Microsoft 365)
- [SOC 2](https://docs.microsoft.com/compliance/regulatory/offering-soc) (Microsoft 365)
- South Africa Consumer Protection ACT 68 2008 (Microsoft 365)
- South Africa Consumer Protection ACT 68 2008 (Microsoft 365)
- South Africa Electronic Communications and Transactions Act, 2002 (Microsoft 365)
- POPIA sudafricano (Microsoft 365)
- Spagna - Framework di sicurezza nazionale (Microsoft 365)
- Controlli di sicurezza dei clienti SWIFT (Microsoft 365)
- Svizzera - Federal Act on Data Protection (FADP) (Microsoft 365)
- Taiwan - Regole di implementazione per il sistema di controllo interno degli istituti di pagamento elettronici - 2015 (Microsoft 365)
- Taiwan - Normative che regolano l'approvazione e l'amministrazione delle aziende del servizio di informazioni finanziarie che si impegnano nel trasferimento e nella risoluzione dei fondi interbanche (Microsoft 365)
- Taiwan - Normative che regolano gli standard per il sistema delle informazioni e la gestione della sicurezza degli istituti di pagamento elettronici (Microsoft 365)
- Taiwan - Regole di implementazione del sistema di controllo interno delle società di holding finanziarie e dei settori bancari (Microsoft 365)
- Taiwan Personal Data Protection Act (PDPA) (Microsoft 365)
- Texas - Identity Theft Enforcement and Protection Act (Microsoft 365)
- Thailandia PDPA (Microsoft 365)
- Trade Secrets Act of The Republic of China (Microsoft 365)
- Trinidad and Tobago Data Protection (Act 13 of 2011) (Microsoft 365)
- [Trusted Information Security Assessment Exchange](https://docs.microsoft.com/compliance/regulatory/offering-tisax-germany)
- Turchia - Protezione KVKK dei dati personali 6698 (Microsoft 365)
- Emirati Uniti - Legge federale n. 2 del 2019 sull'uso della tecnologia delle informazioni e delle comunicazioni (ICT) nei campi sanitari
- Regno Unito - Regolamento per le attività di evasore di endovaso /2011 (Microsoft 365)
- [UK Cyber Essentials](https://docs.microsoft.com/compliance/regulatory/offering-cyber-essentials-plus-uk) (Microsoft 365)
- Regno Unito - Cyber Security for Defense Suppliers Standard (Microsoft 365)
- Privacy e comunicazioni elettroniche del Regno Unito (Microsoft 365)
- Ucraina - Protezione della legge sui dati personali (Microsoft 365)
- US DoE 10 CFR Parte 810 (Microsoft 365)
- US - Federal Information Security Modernization Act del 2014 (FISMA) (Microsoft 365)
- [US FERPA](https://docs.microsoft.com/compliance/regulatory/offering-ferpa) (Microsoft 365)
- US-Cloud Act (Microsoft 365)
- Legge sulla protezione del credito al consumo (Microsoft 365)
- Legge Uzbekistan sui dati personali
- Standard V2.0 (VPDSS 2.0) (Microsoft 365)
- Vietnam - Consumer Rights Protection Law (Microsoft 365)
- Vietnam - Legge sulla cybersecurity (Microsoft 365)
- Vietnam - Legge sulla sicurezza delle informazioni di rete
- Vietnam - Legge sulla tecnologia dell'informazione (Microsoft 365)
- Yemen - Legge del diritto di accesso alle informazioni in Yemen (Microsoft 365)
