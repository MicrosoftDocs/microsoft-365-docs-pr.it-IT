---
title: Ripristinare le impostazioni del produttore per i dispositivi Windows 10
ms.author: sirkkuw
author: Sirkkuw
manager: scotv
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
localization_priority: Normal
ms.custom:
- Core_O365Admin_Migration
- MiniMaven
- MSB365
search.appverid:
- BCS160
- MET150
ms.assetid: c4db6caf-74df-4734-b1dd-53e371c7a3c3
description: 'Informazioni su come utilizzare Microsoft 365 Business per i dispositivi Windows 10 ripristino delle impostazioni. '
ms.openlocfilehash: ed745ea792f755ada17b3b327032246dab16d94b
ms.sourcegitcommit: eb1a77e4cc4e8f564a1c78d2ef53d7245fe4517a
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 11/28/2018
ms.locfileid: "26868812"
---
# <a name="reset-windows-10-devices-to-their-factory-settings"></a>Ripristinare le impostazioni del produttore per i dispositivi Windows 10

Il ripristino delle impostazioni predefinite ripristina le impostazioni originali di un dispositivo, ossia quelle disponibili al momento dell'acquisto. Tutte le app e i dati aggiunti nel dispositivo dopo l'acquisto vengono rimossi. È possibile usare Microsoft 365 Business per eseguire un ripristino delle impostazioni predefinite dei dispositivi Windows 10 gestiti.
  
1. Accedere al [portale di amministrazione](https://aka.ms/bcsportal) con le credenziali di amministratore globale. 
    
2. Nell'interfaccia di amministrazione scegliere **Ripristino delle impostazioni predefinite** nella scheda **Azioni dispositivo**.
    
    ![On the Device actions card, choose Factory reset](media/7caddd12-207e-4c99-b61c-0495fc5f55e3.png)
  
3. Nel riquadro **Ripristino delle impostazioni predefinite** selezionare la casella di controllo accanto al dispositivo da cui si vogliono rimuovere i dati e quindi scegliere **Seleziona**.
    
4. Nel riquadro **Ripristinare le impostazioni predefinite dei dispositivi seguenti?** scegliere **Conferma** \> **Chiudi**.
    
  

