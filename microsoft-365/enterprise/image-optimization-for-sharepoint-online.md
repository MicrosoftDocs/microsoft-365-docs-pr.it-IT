---
title: Ottimizzazione delle immagini per i SharePoint di pubblicazione classica di SharePoint Online
ms.author: kvice
...
ms.locfileid: "52924576"
---
# <a name="image-optimization-for-sharepoint-online-classic-publishing-sites"></a>Ottimizzazione delle immagini per i SharePoint di pubblicazione classica di SharePoint Online

La velocità di caricamento di una pagina Web dipende dalle dimensioni combinate di tutti i componenti necessari per il rendering della pagina, tra cui immagini, HTML, JavaScript e CSS. Le immagini sono un ottimo modo per rendere il sito più accattivante, ma le loro dimensioni possono influire sulle prestazioni. Ottimizzando le immagini con compressione e ridimensionamento e usando sprite, puoi sfalsare gli effetti di immagini molto grandi. Usando SharePoint rendering delle immagini, puoi caricare una singola immagine di grandi dimensioni e visualizzare sezioni dell'immagine che ne consentono il riutilizzo anziché il ricaricamento.

>[!NOTE]
>Questo argomento si applica ai SharePoint di pubblicazione classica online, non ai siti portale moderni. Per informazioni sull'ottimizzazione delle immagini in SharePoint siti portale moderni di SharePoint Online, vedere Ottimizzare le immagini nelle pagine del [portale moderno di SharePoint Online.](modern-image-optimization.md)
  
## <a name="using-sprites-to-speed-up-image-loading"></a>Uso di sprite per velocizzare il caricamento delle immagini

![Screenshot di spcommon](../media/cc5cdee1-8e54-4537-9a8a-8854f4ee849f.png)

Uno sprite immagine contiene molte immagini più piccole. Usando CSS puoi selezionare una parte dell'immagine composita da visualizzare in una determinata parte della pagina con posizionamento assoluto. Fondamentalmente, sposti una singola immagine intorno alla pagina invece di caricare più immagini e rendi visibile una piccola parte dell'immagine attraverso una piccola finestra in cui la parte necessaria dell'immagine sprite viene visualizzata all'utente finale. SharePoint Online usa gli sprite per visualizzare le varie icone nel file spcommon.png sprite.

Cosa è stato trattato qui:
...
> [!NOTE]
> I rendering sono disponibili solo per la SharePoint quando la pubblicazione è abilitata. È possibile abilitare la pubblicazione in Impostazioni \> Sito Impostazioni Gestisci caratteristiche sito SharePoint Server \> \> Publishing. L'opzione non verrà visualizzata in caso contrario.
