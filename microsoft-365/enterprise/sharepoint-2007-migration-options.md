...
---
# <a name="sharepoint-2007-migration-options-to-consider"></a>SharePoint di migrazione 2007 da considerare
...
Esistono diversi metodi per aggiornare un ambiente SharePoint Server. Se si dispone di una farm Microsoft Office SharePoint Server 2007, ecco alcuni esempi dei metodi di aggiornamento:
...
- SharePoint ibridi (connettersi online a SharePoint)
    
- Spostare manualmente i dati tra raccolte siti o raccolte
    
- Aggiornamento guidato FastTrack a Microsoft 365 ([SharePoint advisor per la](https://aka.ms/spoguidance)distribuzione online )
    
- API di migrazione SharePoint Online (SPO) in Microsoft 365
    
Cosa funziona meglio per te?
  
La conoscenza di ciò che la farm fa e per cui viene utilizzata è un punto di forza tattico quando si tratta di eseguire l'aggiornamento. Il modo in cui gli utenti SharePoint farm consentono di scegliere tra le opzioni.
  
> [!TIP]
> Microsoft Office SharePoint Server 2007 ha anche un aggiornamento graduale non trattato qui. Per un elenco di articoli specifici per l'aggiornamento, vedere SharePoint End of Support Roadmap di [SharePoint Server 2007.](sharepoint-2007-end-of-support.md) 
  
Ricordati di controllare il [ciclo di vita del](https://support.microsoft.com/lifecycle/search) prodotto e i requisiti di sistema per qualsiasi versione SharePoint a cui stai eseguendo l'aggiornamento. In questo modo sarà necessario tenere presente quando sarà necessario il prossimo aggiornamento (ad esempio, se si sospende un prodotto legacy come SharePoint Server 2010 per pianificare ulteriori aggiornamenti, assicurarsi di conoscere la data di fine del supporto) e di essere certi di disporre di hardware che supporti il piano. 
  
Se si prevede di eseguire la transizione di alcuni o tutti i siti di SharePoint a Microsoft 365 nel cloud, questo è il momento di aggiungere un segnalibro a un collegamento alle descrizioni dei servizi [Microsoft 365 e Office 365](/office365/servicedescriptions/office-365-service-descriptions-technet-library). Per informazioni sulle funzionalità di SharePoint Online e su come potrebbero differire da SharePoint Server locale, è necessario disporre delle descrizioni dei servizi. Aggiornare le Microsoft Office SharePoint server 2007. Se l'installazione presenta siti interrotti, correggerli prima dell'aggiornamento.
...
2. Se nella farm di Microsoft Office SharePoint Server 2007 è in uso solo un numero limitato di raccolte documenti critiche e altre informazioni, è possibile scegliere di spostare manualmente i dati da Microsoft Office SharePoint Server 2007 a SharePoint Server 2010 oppure di passare solo a siti e Web specifici alla versione successiva (semplificando il processo).
    
3. Minore è il numero di Microsoft Office SharePoint server farm di Server 2007, in modo diretto, più sicuri sono i dati contenuti nella farm durante l'aggiornamento.
    
Metodi come In-Place'aggiornamento agiranno direttamente nella farm di Microsoft Office SharePoint Server 2007, offrendo meno opzioni semplici per abbandonare un percorso e ricominciare con l'ambiente immacolato. Per quanto possibile, è necessario adottare alcune misure di sicurezza, ad esempio l'esecuzione e il testing dei backup dell'ambiente originale. Ad esempio, se la farm di Microsoft Office SharePoint Server 2007 è virtuale e viene duplicata ai fini del backup e del ripristino, eseguire il backup e il ripristino dei database più aggiornati prima della finestra del servizio per l'aggiornamento. Se si ha la possibilità di ripristinare i backup dei database, non solo si avrà un failsafe, ma si potrà anche avere la tranquillità.
...
Se è necessario eseguire l'aggiornamento, è necessario un piano e le dimensioni una non sono adatte a tutti in questi casi. Il piano può essere semplice come "Creare una sottoscrizione Microsoft 365 con SharePoint Online, registrare un dominio e reindirizzare le persone a salvare i propri file lì". E potrebbe non essere così. Questa decisione è tua e si tratta di ciò di cui hai realmente bisogno tu e i tuoi utenti.
...
Durante l'aggiornamento, il processo decisionale deve essere basato sulle esigenze della farm per l'organizzazione. Qual è la necessità che soddisfi? Qual è il suo ruolo? Ogni farm dell'azienda può avere un ruolo diverso. Alcune delle SharePoint farm possono *essere* critiche, altre potrebbero essere archivi di file, per garantire la sicurezza. In caso contrario, se la farm occupa molti ruoli contemporaneamente, potrebbe essere necessario conoscere le raccolte siti, i Web o persino le raccolte documenti, eventuali personalizzazioni e quanto sono importanti. L'analisi dei dati a questo livello può sembrare molto lavoro, ma consente di risparmiare tempo e fatica per la gestione del dominio prima di aggiornarlo o eseguirne la migrazione. Una volta che conosci tutte le parti in movimento e i bit più importanti, saprai anche cosa hai superato e puoi lasciarti alle spalle. Questa conoscenza sarà utile solo per il futuro. 
  
Ciò che gli utenti affermano, quindi, è più importante per la SharePoint Server farm?
  
- Funzionalità di SharePoint integrate
...
Se esegui qualcosa di essenziale per la tua azienda dalla farm di SharePoint, diciamo che funziona come un grande catalogo di dati critici sui requisiti del servizio client, puoi inserire un segno di spunta accanto a "App critiche", ma anche "Disponibilità", cio? l'azienda sarebbe influenzata se non riuscisse a usare SharePoint per un po'. Analogamente, è possibile controllare "Personalizzazioni" perché i servizi critici offerte dalla farm si basano su codice personalizzato, definizioni di sito o una serie di personalizzazioni che funzionano insieme.
...
|API di migrazione a SharePoint Online Microsoft 365 (per i dati dei siti personali)  <br/> |SharePoint Ibrido (non ancora necessario)  <br/> |
|Alcune migrazioni manuali dei dati a SharePoint Online per i dati critici  <br/> |Aggiornamento guidato FastTrack a Microsoft 365  <br/> |
   
 **Piano proposto:**
  
Eseguire l'aggiornamento in locale, con versioni di SharePoint affiancate, alcune virtualizzate, in modo da poter aggiornare prima i database. Passare da SharePoint 2007 a SharePoint 2010. Gli amministratori e gli sviluppatori testano la farm risultante. Gli utenti testano la farm risultante. Consente di risolvere eventuali problemi di interruzione della visualizzazione durante questo periodo. Anche in questo caso, eseguire l'aggiornamento affiancato SharePoint 2010 a SharePoint 2013. Test. Test utente/pilota. Consente di risolvere eventuali problemi di interruzione della visualizzazione durante questo periodo.
...
- Si stima che il 35% dei siti possa essere trasformato in nuovi siti di SharePoint Online con domini di vanità o, in ultima analisi, diventare OneDrive for Business archiviazione. Ricerca di altre opportunità per convertire siti o instradare nuovi siti a SharePoint Online.
    
- Alcune di queste parti della migrazione saranno manuali, tramite trascinamento della selezione OneDrive for Business siti personali e altre tramite l'API di migrazione.
...
Spesso i fattori principali nella scelta di un metodo sono il costo totale dell'aggiornamento e il costo nel tempo (vedere di più nell'articolo sulla roadmap di migrazione di SharePoint). Tuttavia, la pianificazione in futuro sarà molto utile per impostare le aspettative, scegliere con sapiente e inquadrare l'aspetto del successo.
