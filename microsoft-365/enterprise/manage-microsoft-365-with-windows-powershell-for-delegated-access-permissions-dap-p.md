...
I partner di autorizzazione accesso delegato (DAP, Delegated Access Permission) sono partner di Syndication e Cloud Solution Provider (CSP). Molti sono provider di rete o di telecomunicazioni. Aggregano le sottoscrizioni di Microsoft 365 nelle offerte di servizi. Quando vendono un abbonamento a Microsoft 365, vengono automaticamente concesse le autorizzazioni Amministra per conto di (AOBO) ai tenaancies del cliente in modo che possano amministrare e segnalare tali tenanze. Queste attività sono difficili da eseguire nell'interfaccia di amministrazione di Microsoft 365. È molto più semplice usare PowerShell per Microsoft 365 per eseguire attività amministrative quali:
- Elencare tutti i **TenantId dei clienti** e i relativi domini 
- Identificare tutti gli utenti in una tenancy del cliente e le licenze assegnate
> [!NOTE]
...
- [Gestire i tenant di Microsoft 365 con Windows PowerShell per i partner daP (Delegated Access Permissions)](manage-microsoft-365-tenants-with-windows-powershell-for-delegated-access-permissio.md)
