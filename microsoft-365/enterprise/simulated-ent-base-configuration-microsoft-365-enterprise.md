...
- Intranet dell'organizzazione semplificata connessa a Internet, costituita da tre macchine virtuali in una rete virtuale di Azure (DC1, APP1 e CLIENT1).
...
La creazione di un ambiente di test semplificato prevede due fasi:
...
In questo metodo viene utilizzato un modello di Azure Resource Manager per creare la rete Intranet simulata. I modelli di Azure Resource Manager contengono tutte le istruzioni per creare l'infrastruttura di rete di Azure, le macchine virtuali e la relativa configurazione.
...
Configurare quindi DC1 come controller di dominio e server DNS per il dominio **testlab.**\<*your public domain*> . Specificare il nome di dominio pubblico, rimuovere le parentesi angolari ed eseguire questi comandi a livello di amministratore Windows PowerShell prompt dei comandi in DC1.
...
Per creare una macchina virtuale di Azure per CLIENT1, immettere il nome del gruppo di risorse ed eseguire questi comandi al prompt dei comandi nel computer locale.
...
2. In **Proprietà per CLIENT1** selezionare **Attiva** accanto a Configurazione sicurezza avanzata di **Internet Security**.
...
4. Nella schermata Start seleziona **Internet Explorer** e quindi seleziona **OK.**
...
- Usare un abbonamento di produzione separato a Microsoft 365 E5 con un numero limitato di licenze.
  ...
  Si tratta di un costo aggiuntivo, ma garantisce che l'ambiente di testing funzionante non scada; in esso, è possibile provare funzionalità, configurazioni e scenari. È possibile utilizzare lo stesso ambiente di test a lungo termine per le prove di concetto, la dimostrazione per i peer e la gestione e lo sviluppo e il testing delle applicazioni. Si tratta del metodo consigliato.
