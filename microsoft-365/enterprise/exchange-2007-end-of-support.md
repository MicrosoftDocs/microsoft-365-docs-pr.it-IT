...
- Eseguire la migrazione a Microsoft 365 tramite la migrazione completa, a fasi o ibrida.
...
La migrazione della posta elettronica a Microsoft 365 è l'opzione più semplice e ottimale per ritirare la distribuzione di Exchange 2007. Con una migrazione a Microsoft 365, è possibile eseguire un singolo passaggio dalla tecnologia di 10 anni a funzionalità all'avanguardia, tra cui:
...
Microsoft 365 ottiene anche nuove funzionalità ed esperienze, in modo che tu e gli utenti in genere possano iniziare subito a usarle. E non dovrai preoccuparti di:
...
- Aggiornamento a una nuova versione di Exchange. Con Microsoft 365, si è sempre sulla versione più recente di Exchange.
...
La migrazione a fasi è ideale per le organizzazioni che devono richiedere più tempo per eseguire la migrazione delle proprie cassette postali a Microsoft 365, ma pianificano comunque di completare la migrazione entro poche settimane. È possibile eseguire la migrazione delle cassette postali in batch. È possibile controllare il numero e le cassette postali di cui viene eseguita la migrazione in un determinato momento. È possibile, ad esempio, batch di cassette postali di utenti nello stesso reparto per assicurarsi che siano tutti spostati contemporaneamente. In caso contrario, è possibile lasciare le cassette postali executive fino all'ultimo batch. Come per le migrazioni completate, gli utenti dovranno ricreare i profili di Outlook.
...
- I domini accettati di Exchange 2007 che si prevede di utilizzare in Microsoft 365 devono essere aggiunti come domini verificati nel servizio.
    
- Sarà necessario creare un file CSV con il nome completo e l'indirizzo di posta elettronica di ogni cassetta postale di cui si prevede di eseguire la migrazione in un batch. Sarà inoltre necessario includere una nuova password per ogni cassetta postale di cui si sta eseguendo la migrazione e inviare tale password a ogni utente. All'utente verrà richiesto di modificare la password al primo accesso alla nuova cassetta postale di Microsoft 365.
    
- Tra l'avvio del batch di migrazione e la fase di completamento, Microsoft 365 sincronizza periodicamente le cassette postali di Microsoft 365 e locali incluse nel batch. In questo modo è possibile completare la migrazione senza preoccuparsi che la posta elettronica venga lasciata nelle cassette postali locali.
...
- Mostrare agli utenti locali le informazioni sulla disponibilità del calendario per gli utenti in Microsoft 365 e viceversa.
    
- Vedere un elenco indirizzi globale unificato contenente destinatari sia in locale che in Microsoft 365.
...
- Proteggere le comunicazioni di posta elettronica tra i server Exchange locali e Microsoft 365 utilizzando TLS e certificati.
...
- Sarà necessario distribuire almeno un server Exchange 2013 nell'organizzazione di Exchange 2007 per agire come "server ibrido". Questo server comunicherà con Microsoft 365 per conto dei server Exchange 2007.
    
- Microsoft 365 dovrà connettersi al "server ibrido" utilizzando Outlook via Internet sulla porta TCP 443.
    
- Dovrai configurare la sincronizzazione della directory usando Azure Active Directory (Azure AD) Connect tra i server Active Directory locali e Microsoft 365.
    
- Gli utenti potranno accedere alla propria cassetta postale di Microsoft 365 utilizzando lo stesso nome utente e la stessa password dell'accesso alla rete locale. Questa funzionalità richiede Azure AD Connect con sincronizzazione delle password e/o Active Directory Federation Services.
...
- [Assistente per la distribuzione di Exchange](/exchange/exchange-deployment-assistant)
...
|**Rilascio di Exchange**|**Caratteristiche**|
|:-----|:-----|
...
|:-----|:-----|
|Date di fine supporto  <br/> | Come Exchange 2007, ogni versione di Exchange ha la propria data di fine del supporto:  <br/> *Exchange 2010* - Gennaio 2020  <br/> *Exchange 2013* - Aprile 2023  <br/> *Exchange 2016* - Ottobre 2025  <br/>  Prima della fine del supporto, prima sarà necessario eseguire un'altra migrazione.<br/> |
...
Se si verificano problemi durante la migrazione a Microsoft 365 e non si utilizza FastTrack o la migrazione a una versione più recente di Exchange Server, siamo qui per aiutarti. Ecco alcune risorse che puoi usare:
