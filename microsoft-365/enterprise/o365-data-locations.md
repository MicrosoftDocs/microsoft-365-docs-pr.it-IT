---
title: Posizioni dei dati di Microsoft 365
ms.author: deanw
author: dmwmsft
manager: laurawi
audience: ITPro
ms.topic: article
ms.service: o365-solutions
localization_priority: Priority
search.appverid:
- MET150
ms.collection:
- Ent_O365
- Strat_O365_Enterprise
ms.custom:
- Ent_TLGs
ms.assetid: 706d5449-45e5-4b0c-a012-ab60501899ad
description: 'Determinare dove sono archiviati i dati dei clienti Microsoft 365 in tutto il mondo '
ms.openlocfilehash: 4b1e0d30d37ec9bc7a333216854e8eb602e07cc1
ms.sourcegitcommit: d3ca8021f7da00a474ac14aac5f1358204a848f2
ms.translationtype: HT
ms.contentlocale: it-IT
ms.lasthandoff: 12/01/2020
ms.locfileid: "49519728"
---
<!--
Note to new owner: Links in the Notes and FAQ sections need to be added from the source document at https://microsoft.sharepoint.com/:w:/t/golocalteam/Ebv6_Lw6FvNKucfA1eESTPUBy6O1MdvWEeT-L8O5a2HsIg?e=4%3AUfF1Uj&at=9&xsdata=&sdata=MkxOUXhlZEdyR0I1SzBvd044d2hkd2RPaGN2RUpaS09jMkZTcys0dkZQRT0%3D
-->

# <a name="where-your-microsoft-365-customer-data-is-stored"></a>Dove sono archiviati i dati dei clienti Microsoft 365

Le tabelle seguenti mostrano le posizioni in cui sono archiviati i dati inattivi dei clienti per i servizi di Microsoft 365 in tutte le aree geografiche del cloud globale di Microsoft. Espandere la posizione del paese in cui si trova l'indirizzo di fatturazione per scoprire dove verranno archiviati i dati dei clienti per ogni servizio.

Se l'organizzazione si trova nell'Unione europea, per altre informazioni, vedere [Posizioni dei dati per l'Unione europea](EU-data-storage-locations.md). 

I clienti possono visualizzare informazioni sulla posizione di dati specifici del tenant nell'interfaccia di amministrazione di Microsoft 365 in **Impostazioni | Impostazioni organizzazione | Profilo organizzazione | Percorso dati**.

Per impostazione predefinita, i nuovi tenant di Microsoft 365 vengono ospitati in un'area geografica in base al paese della transazione associata al primo abbonamento del tenant.

Informazioni sugli impegni contrattuali relativi alla posizione di archiviazione dei dati inattivi dei clienti sono disponibili nelle [Condizioni di Microsoft Online Services (OST)](https://go.microsoft.com/fwlink/p/?LinkId=2098215).

Per le posizioni dei dati di Azure Active Directory, visitare la [mappa dei dati di AAD](https://go.microsoft.com/fwlink/p/?linkid=2092972).

Se l’indirizzo di fatturazione del cliente è al di fuori dell’Europa e il cliente ha un abbonamento a Office 365 Education, nonostante la sezione “Posizione dei dati del cliente a riposo per i principali servizi online” dell’OST, Microsoft può fornire il tenant di Office 365 del cliente, trasferire i dati del cliente e archiviare i dati del cliente a riposo in qualsiasi parte dell’Europa o del Nord America. Se l’indirizzo di fatturazione del cliente si trova in Europa e il cliente ha un abbonamento a Office 365 Education, nonostante la sezione “Posizione dei dati del cliente a riposo per i servizi online principali” dell’OST, Microsoft può fornire il tenant di Office 365 del cliente, trasferire i dati del cliente e archiviare i dati del cliente a riposo in qualsiasi parte dell’Unione Europea.




## <a name="faq"></a>Domande frequenti

### <a name="how-does-microsoft-define-data"></a>In che modo Microsoft definisce i dati?

Esaminare le [definizioni dei diversi tipi di dati](https://go.microsoft.com/fwlink/p/?linkid=864390) nel Centro protezione Microsoft. Nei [Termini di Microsoft Online Services ](https://go.microsoft.com/fwlink/p/?linkid=862403), Microsoft si impegna contrattualmente a fornire i dati dei clienti/dati personali. Ci riferiamo ai dati principali del cliente come ai dati del cliente che si impegna ad archiviare a riposo solo all’interno dell’area geografica in base ai [Termini di Microsoft Online Services](https://go.microsoft.com/fwlink/p/?linkid=862403).

### <a name="where-are-the-exact-addresses-of-the-data-centers"></a>Quali sono gli indirizzi esatti dei data center?

Microsoft non rivela gli indirizzi esatti dei suoi data center. Abbiamo stabilito questa politica per proteggere le nostre strutture del data center. Tuttavia, elenchiamo le posizioni delle città. Per ulteriori informazioni, vedere [Posizioni dei data center](o365-data-locations.md#data-center-locations).

### <a name="does-the-location-of-your-customer-data-have-a-direct-impact-on-your-end-users-experience"></a>La posizione dei dati dei clienti ha un impatto diretto sull'esperienza degli utenti finali?

Le prestazioni di Microsoft 365 non sono semplicemente proporzionali alla distanza di un utente dalle posizioni dei data center. I continui investimenti di Microsoft nella rete cloud globale, nell’infrastruttura cloud globale e nell’architettura dei servizi Microsoft 365 consentono di fornire agli utenti un’esperienza unica e coerente, indipendentemente da dove sono archiviati i dati dei clienti a riposo. Se gli utenti riscontrano problemi di prestazioni, è necessario risolverli in modo approfondito. Microsoft ha pubblicato indicazioni per i clienti di Microsoft 365 per pianificare e ottimizzare le prestazioni degli utenti finali nel [sito Web del supporto tecnico di Office](https://go.microsoft.com/fwlink/p/?linkid=862645).

### <a name="how-does-microsoft-help-me-comply-with-my-national-regional-and-industry-specific-regulations"></a>In che modo Microsoft aiuta i clienti a rispettare la conformità a normative nazionali, regionali e specifiche del settore?

Per aiutare l’utente a rispettare i requisiti nazionali, regionali e specifici del settore che regolano la raccolta e l’utilizzo dei dati individuali, Microsoft 365 offre il set più completo di conformità di qualsiasi provider di produttività cloud globale. Esaminare le [offerte di conformità](https://go.microsoft.com/fwlink/p/?linkid=864391) e ulteriori dettagli nella sezione [Conformità di Microsoft 365](https://go.microsoft.com/fwlink/p/?linkid=862317) nel Centro protezione Microsoft. Inoltre, alcuni piani Microsoft 365 offrono ulteriori soluzioni di conformità per aiutare a gestire i dati, rispettare i requisiti legali e normativi e monitorare le azioni intraprese con i dati.

### <a name="who-can-access-your-data-and-according-to-what-rules"></a>Chi può accedere ai dati e in base a quali regole?

 Microsoft implementa misure efficaci per proteggere i dati dei clienti da accessi o usi inappropriati da parte di persone non autorizzate. Ciò include la limitazione dell’accesso da parte del personale e dei subappaltatori Microsoft e la definizione attenta dei requisiti per rispondere alle richieste governative di dati dei clienti. Tuttavia, è possibile accedere ai dati dei clienti in qualsiasi momento e per qualsiasi motivo. Ulteriori dettagli sono disponibili nel  [Centro protezione Microsoft](https://go.microsoft.com/fwlink/p/?linkid=864392).

### <a name="does-microsoft-access-your-data"></a>Microsoft accede ai dati?

Microsoft automatizza la maggior parte delle operazioni di Microsoft 365 limitando intenzionalmente il proprio accesso ai dati dei clienti. Questo ci aiuta a gestire Microsoft 365 su larga scala e ad affrontare i rischi di minacce interne ai dati dei clienti. Per impostazione predefinita, gli ingegneri Microsoft non hanno privilegi amministrativi permanenti e nessun accesso permanente ai dati dei clienti in Microsoft 365. Un ingegnere Microsoft può avere accesso limitato e registrato ai dati dei clienti per un periodo di tempo limitato, ma solo quando è necessario per le normali operazioni di servizio e solo se approvato da un membro del senior management di Microsoft (e, nel caso di clienti che dispongono della licenza per la funzionalità Customer Lockbox, da parte del cliente).

### <a name="how-does-microsoft-secure-your-data"></a>In che modo Microsoft protegge i dati?

Microsoft dispone di criteri, controlli e sistemi affidabili integrati in Microsoft 365 per proteggere le informazioni. Esaminare la [sezione sulla sicurezza di Microsoft 365](https://go.microsoft.com/fwlink/p/?linkid=864393) nel Centro protezione Microsoft per saperne di più. 

### <a name="does-microsoft-365-encrypt-your-data"></a>Microsoft 365 crittografa i dati?

Microsoft 365 utilizza tecnologie di servizio che crittografano i dati dei clienti a riposo o in transito, Per i dati a riposo, Microsoft 365 utilizza la crittografia a livello di volume e file, Per i dai dei clienti in transito, Microsoft 365 utilizza tecnologie di crittografia multiple per le comunicazioni tra data center e tra client e server, come Transport Layer Security (TLS) e Internet Protocol Security (IPsec). Microsoft 365 include anche funzionalità di crittografia gestite dal cliente.

### <a name="where-can-i-find-data-residency-information-for-microsoft-azure"></a>Dove è possibile trovare informazioni sulla residenza dei dati per Microsoft Azure?

Consultare la pagina [Prodotti disponibili in base all'area](https://go.microsoft.com/fwlink/p/?linkid=2093451) per informazioni sulla residenza dei dati per Microsoft Azure.

### <a name="what-are-multi-geo-capabilities-in-microsoft-365"></a>Cos'è Multi-Geo Capabilities in Microsoft 365?

[Il multi-Geo](https://go.microsoft.com/fwlink/p/?linkid=872033) offre ai clienti la possibilità di assegnare i dati di base ai clienti nelle posizioni disponibili nel cloud Microsoft 365.   

### <a name="what-services-support-multi-geo"></a>Quali servizi supportano Multi-Geo?

[Multi-Geo](https://go.microsoft.com/fwlink/p/?linkid=872033) è disponibile per Exchange Online, OneDrive e SharePoint Online. Microsoft sta esaminando Multi-Geo per altri servizi Microsoft 365.

### <a name="what-are-the-exceptions-for-intune-data-locations"></a>Quali sono le eccezioni per i percorsi dei dati di Intune?

Microsoft non memorizzerà i dati dei clienti di Intune all'esterno della posizione geografica indicata. Eccetto se:

- È necessario che Microsoft fornisca assistenza clienti, risolva i problemi del servizio o si conformi ai requisiti legali.
- Il cliente configura un account per abilitare tale spazio di archiviazione per i dati dei clienti, anche usando le seguenti operazioni:

  - Caratteristiche che sono pensate per operare globalmente, come la Rete per la distribuzione di contenuti (CDN), che offre un servizio di Coaching globale e memorizza i dati dei clienti in posizioni perimetrali in tutto il mondo.

  - Azure Active Directory (Azure AD), che potrebbe archiviare i dati della directory globalmente. Questa operazione non è valida per le distribuzioni di Azure Active Directory negli Stati Uniti (dove i dati di Azure Active Directory sono archiviati solo negli Stati Uniti) o in Europa (in cui i dati di Azure Active Directory sono archiviati in Europa o Stati Uniti).
   
  - Preview, beta o altri servizi provvisori, che in genere memorizzano i dati dei clienti negli Stati Uniti, ma possono archiviarli globalmente.

Indipendentemente da ciò, Microsoft non controlla o limita l’area geografica da cui i clienti o gli utenti finali possono accedere ai dati dei clienti. 

## <a name="data-center-locations"></a>Posizioni dei data center

La tabella seguente definisce la posizione dei dati per i vari servizi. Per determinare le posizioni dei servizi, fare riferimento a queste tabelle.

### <a name="data-center-geographies"></a>Aree geografiche dei data center

Le aree geografiche globali seguenti possono archiviare dati inattivi. Le posizioni in cui possono essere archiviati i dati dei clienti possono cambiare. 

| Aree geografiche globali | Posizioni in cui possono essere archiviati i dati dei clienti |
| --- | --- |
| Area geografica globale 1-EMEA (Europa, Medio Oriente e Africa) | Austria, Finlandia, Francia, Irlanda, Paesi Bassi |
| Area geografica globale 2 - Asia Pacifico | RAS di Hong Kong, Giappone, Malaysia, Singapore, Corea del Sud |
| Area geografica globale 3 - Americhe | Brasile, Cile, Stati Uniti |

## <a name="countryregion-specific-data-center-city-locations"></a>Città di ubicazione dei data center specifici per paese/area geografica

Per i data center specifici per paese o area geografica, di seguito sono indicate le città in cui vengono archiviati i dati inattivi dei clienti.

| Paese | Città |
| --- | --- |
| Australia | Sydney, Melbourne |
| Brasile | Rio, Campinas |
| Canada | Québec, Toronto |
| Unione Europea | Austria (Vienna), Finlandia (Helsinki), Francia (Parigi, Marsiglia), Irlanda (Dublino), Paesi Bassi (Amsterdam) |
| Francia | Parigi, Marsiglia |
| Germania | Francoforte, Berlino |
| India | Chennai, Mumbai, Pune |
| Giappone | Osaka, Tokyo, Saitama |
| Corea del Sud | Busan, Seoul |
| Norvegia | Oslo, Stavanger |
| Sudafrica | Città del Capo, Johannesburg |
| Svizzera | Ginevra, Zurigo |
| Emirati Arabi Uniti | Dubai, Abu Dhabi |
| Regno Unito | Durham, Londra, Cardiff |
| Stati Uniti |  Boydton, Cheyenne, Chicago, Des Moines, Quincy, San Antonio, Santa Clara, San Jose |

Per impostazione predefinita, i nuovi tenant di Microsoft 365 vengono ospitati in un'area geografica in base al paese dell'indirizzo di fatturazione associato al primo abbonamento del tenant. Per confermare dove sono archiviati i dati del cliente per i servizi di Office 365 esistenti, visualizzare le informazioni sulla posizione dei dati specifici del tenant nell’interfaccia di amministrazione di Office 365 in Impostazioni | Impostazioni organizzazione | Profilo dell’organizzazione | Scheda posizione dati.  In alternativa, espandere il paese in cui si trova l’azienda nell’elenco di seguito per scoprire dove verrebbero archiviati i dati dei clienti se si iniziasse a usare un servizio Microsoft 365 oggi stesso.

## <a name="afghanistan"></a>Afghanistan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="aland-islands"></a>Isole Åland
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="albania"></a>Albania
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="algeria"></a>Algeria
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="american-samoa"></a>Samoa americane
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="andorra"></a>Andorra
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="angola"></a>Angola
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="anguilla"></a>Anguilla
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="antarctica"></a>Antartide
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="antigua-and-barbuda"></a>Antigua e Barbuda
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="argentina"></a>Argentina
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="armenia"></a>Armenia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="aruba"></a>Aruba
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="australia"></a>Australia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Australia |
| OneDrive for Business | Australia |
| SharePoint Online | Australia |
| Skype for Business | Australia |
| Microsoft Teams | Australia |
| Office Online &amp; Mobile | Australia |
| EOP | Australia |
| Intune | Australia |
| MyAnalytics | Australia |
| Planner | Australia |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Australia |
| Stream | Australia |
| Whiteboard | Australia |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="austria"></a>Austria
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="azerbaijan"></a>Azerbaigian
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner |  Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bahamas"></a>Bahamas
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bahrain"></a>Bahrein
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bangladesh"></a>Bangladesh
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="barbados"></a>Barbados
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="belarus"></a>Bielorussia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="belgium"></a>Belgio
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="belize"></a>Belize
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="benin"></a>Benin
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bermuda"></a>Bermuda
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bhutan"></a>Bhutan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bolivia"></a>Bolivia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bonaire"></a>Bonaire
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bosnia"></a>Bosnia ed Erzegovina
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner |  Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="botswana"></a>Botswana
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bouvet-island"></a>Isola Bouvet
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="brazil"></a>Brasile
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Brasile |
| OneDrive for Business | Brasile |
| SharePoint Online | Brasile |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Brasile |
| Office Online &amp; Mobile | Brasile |
| Exchange Online Protection | Brasile |
| Intune | Stati Uniti |
| MyAnalytics | Brasile |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Brasile |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="british-indian-ocean-territory"></a>Territorio britannico dell'Oceano Indiano
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="british-virgin-islands"></a>Isole Vergini Britanniche
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="brunei"></a>Brunei
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="bulgaria"></a>Bulgaria
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="burkina-faso"></a>Burkina Faso
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="burundi"></a>Burundi
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cambodia"></a>Cambogia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cameroon"></a>Camerun
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="canada"></a>Canada
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Canada |
| OneDrive for Business | Canada |
| SharePoint Online | Canada |
| Skype for Business | Canada |
| Microsoft Teams | Canada |
| Office Online &amp; Mobile | Canada |
| EOP | Canada |
| Intune | Stati Uniti |
| MyAnalytics | Canada |
| Planner | Canada |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Canada |
| Stream | Canada |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cape-verde"></a>Cabo Verde
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cayman-islands"></a>Isole Cayman
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="central-african-republic"></a>Repubblica Centrafricana
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="chad"></a>Ciad
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="chile"></a>Cile
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="china"></a>Cina
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="christmas-island"></a>Isola Christmas
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cocos-keeling-islands"></a>Isole Cocos (Keeling)
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="colombia"></a>Colombia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="comoros"></a>Comore
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="congo-brazzaville"></a>Congo (Brazzaville)
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="congo-kinshasa"></a>Congo, (Kinshasa)
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cook-islands"></a>Isole Cook
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="costa-rica"></a>Costa Rica
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cote-divoire"></a>Costa d'Avorio
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="croatia"></a>Croazia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="curacao"></a>Curaçao
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="cyprus"></a>Cipro
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="czech-republic"></a>Repubblica Ceca
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="denmark"></a>Danimarca
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="djibouti"></a>Gibuti
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="dominica"></a>Dominica
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="dominican-republic"></a>Repubblica Dominicana
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="ecuador"></a>Ecuador
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="egypt"></a>Egitto
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="el-salvador"></a>El Salvador
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="equatorial-guinea"></a>Guinea Equatoriale
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="eritrea"></a>Eritrea
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="estonia"></a>Estonia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="ethiopia"></a>Etiopia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner |  Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="falkland-islands-malvinas"></a>Isole Falkland
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="faroe-islands"></a>Isole Fær Øer
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="federated-states-of-micronesia"></a>Stati federati di Micronesia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="fiji"></a>Figi
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Australia |
| OneDrive for Business | Australia |
| SharePoint Online | Australia |
| Skype for Business | Australia |
| Microsoft Teams | Australia |
| Office Online &amp; Mobile | Australia |
| EOP | Australia |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Australia |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Australia |
| Stream | Australia |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="finland"></a>Finlandia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="france"></a>Francia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Francia |
| OneDrive for Business | Francia |
| SharePoint Online | Francia |
| Skype for Business | Unione Europea |
| Microsoft Teams | Francia |
| Office Online &amp; Mobile | Francia |
| EOP | Francia |
| Intune | Unione Europea |
| MyAnalytics | Francia |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea |
| Servizi di OneNote | Francia |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="french-guiana"></a>Guyana francese
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="french-polynesia"></a>Polinesia francese
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="french-southern-territories"></a>Terre australi francesi
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="gabon"></a>Gabon
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="gambia"></a>Gambia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="georgia"></a>Georgia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="germany"></a>Germania
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Germania |
| OneDrive for Business | Germania |
| SharePoint Online | Germania |
| Skype for Business | Unione Europea |
| Microsoft Teams | Germania |
| Office Online &amp; Mobile | Germania |
| EOP | Germania |
| Intune | Unione Europea |
| MyAnalytics | Germania |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea |
| Servizi di OneNote | Germania |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="ghana"></a>Ghana
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="gibraltar"></a>Gibilterra
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="greece"></a>Grecia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="greenland"></a>Groenlandia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="grenada"></a>Grenada
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="guadeloupe"></a>Guadalupa
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="guam"></a>Guam
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="guatemala"></a>Guatemala
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="guernsey"></a>Guernsey
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="guinea"></a>Guinea
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="guinea-bissau"></a>Guinea-Bissau
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="guyana"></a>Guyana
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="haiti"></a>Haiti
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="heard-and-mcdonald-islands"></a>Isole Heard e McDonald
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="herzegovina"></a>Bosnia ed Erzegovina
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="holy-see-vatican-city-state"></a>Santa Sede (Stato della Città del Vaticano)
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="honduras"></a>Honduras
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="hong-kong-sar"></a>RAS di Hong Kong
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="hungary"></a>Ungheria
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="iceland"></a>Islanda
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="india"></a>India
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | India |
| OneDrive for Business | India |
| SharePoint Online | India |
| Skype for Business | India |
| Microsoft Teams | India |
| Office Online &amp; Mobile | India |
| EOP | India |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | India |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | India |
| Stream | India |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="indonesia"></a>Indonesia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="iraq"></a>Iraq
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="ireland"></a>Irlanda
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="isle-of-man"></a>Isola di Man
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="israel"></a>Israele
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="italy"></a>Italia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="jamaica"></a>Giamaica
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="japan"></a>Giappone
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Giappone |
| OneDrive for Business | Giappone |
| SharePoint Online | Giappone |
| Skype for Business | Giappone |
| Microsoft Teams | Giappone |
| Office Online &amp; Mobile | Giappone |
| EOP | Giappone |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Giappone |
| Planner | Giappone |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Giappone |
| Stream | Giappone |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="jersey"></a>Jersey
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer |  Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="jordan"></a>Giordania
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="kazakhstan"></a>Kazakistan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="kenya"></a>Kenya
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="kiribati"></a>Kiribati
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="south-korea"></a>Corea del Sud
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Corea del Sud |
| OneDrive for Business | Corea del Sud |
| SharePoint Online | Corea del Sud |
| Skype for Business | Corea del Sud |
| Microsoft Teams | Corea del Sud |
| Office Online &amp; Mobile | Corea del Sud |
| EOP | Corea del Sud |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Corea del Sud |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Corea del Sud |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="kosovo"></a>Kosovo
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="kuwait"></a>Kuwait
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="kyrgyzstan"></a>Kirghizistan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="laos"></a>Laos
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="latvia"></a>Lettonia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="lebanon"></a>Libano
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer |  Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="lesotho"></a>Lesotho
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="liberia"></a>Liberia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="libya"></a>Libia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="liechtenstein"></a>Liechtenstein
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Svizzera |
| OneDrive for Business | Svizzera |
| SharePoint Online | Svizzera |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Svizzera |
| Office Online &amp; Mobile | Svizzera |
| EOP | Svizzera |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Svizzera |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Svizzera |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="lithuania"></a>Lituania
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="luxembourg"></a>Lussemburgo
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="madagascar"></a>Madagascar
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="malawi"></a>Malawi
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="malaysia"></a>Malaysia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="maldives"></a>Maldive
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="mali"></a>Mali
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="malta"></a>Malta
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="marshall-islands"></a>Isole Marshall
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="martinique"></a>Martinica
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="mauritania"></a>Mauritania
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="mauritius"></a>Mauritius
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="mayotte"></a>Mayotte
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="mexico"></a>Messico
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="moldova"></a>Moldova
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="monaco"></a>Monaco
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="mongolia"></a>Mongolia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="montenegro"></a>Montenegro
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="montserrat"></a>Montserrat
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="morocco"></a>Marocco
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="mozambique"></a>Mozambico
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="myanmar"></a>Myanmar
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="namibia"></a>Namibia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="nauru"></a>Nauru
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="nepal"></a>Nepal
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="netherlands"></a>Paesi Bassi
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="netherlands-antilles"></a>Antille Olandesi
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="new-caledonia"></a>Nuova Caledonia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="new-zealand"></a>Nuova Zelanda
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Australia |
| OneDrive for Business | Australia |
| SharePoint Online | Australia |
| Skype for Business | Australia |
| Microsoft Teams | Australia |
| Office Online &amp; Mobile | Australia |
| EOP | Australia |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Australia |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Australia |
| Stream | Australia |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="nicaragua"></a>Nicaragua
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="niger"></a>Niger
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="nigeria"></a>Nigeria
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="niue"></a>Niue
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="norfolk-island"></a>Isola Norfolk
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="northern-mariana-islands"></a>Isole Marianne settentrionali
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="norway"></a>Norvegia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Norvegia |
| OneDrive for Business | Norvegia |
| SharePoint Online | Norvegia |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Norvegia |
| Office Online &amp; Mobile | Norvegia |
| EOP | Norvegia |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Norvegia |
| Programmazione | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Norvegia |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Area geografica globale 1 - EMEA |
| Moduli | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="oman"></a>Oman
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="pakistan"></a>Pakistan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="palau"></a>Palau
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="palestinian-authority"></a>Autorità Palestinese
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="panama"></a>Panamá
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="papua-new-guinea"></a>Papua Nuova Guinea
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="paraguay"></a>Paraguay
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="peru"></a>Perù
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="philippines"></a>Filippine
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="pitcairn"></a>Isole Pitcairn
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="poland"></a>Polonia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="portugal"></a>Portogallo
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="puerto-rico"></a>Portorico
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="qatar"></a>Qatar
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="republic-of-macedonia"></a>Repubblica di Macedonia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="runion"></a>Riunione
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="romania"></a>Romania
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="russian-federation"></a>Federazione russa
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="rwanda"></a>Ruanda
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saint-helena"></a>Sant'Elena
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saint-kitts-and-nevis"></a>Saint Kitts e Nevis
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saint-lucia"></a>Saint Lucia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saint-martin"></a>Saint Martin
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saint-pierre-and-miquelon"></a>Saint-Pierre e Miquelon
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saint-vincent-and-the-grenadines"></a>Saint Vincent e Grenadine
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saint-barthlemy"></a>Saint-Barthélemy
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="samoa"></a>Samoa
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="san-marino"></a>San Marino
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="sao-tome-and-principe"></a>São Tomé e Príncipe
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="saudi-arabia"></a>Arabia Saudita
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Area geografica globale 1 - EMEA |
| Moduli | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="senegal"></a>Senegal
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="serbia"></a>Serbia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="seychelles"></a>Seychelles
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="sierra-leone"></a>Sierra Leone
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="singapore"></a>Singapore
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="sint-maarten"></a>Sint Maarten
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="slovakia"></a>Slovacchia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="slovenia"></a>Slovenia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="solomon-islands"></a>Isole Salomone
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="somalia"></a>Somalia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="south-africa"></a>Sudafrica
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Sudafrica |
| OneDrive for Business | Sudafrica |
| SharePoint Online | Sudafrica |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Sudafrica |
| Office Online &amp; Mobile | Sudafrica |
| EOP | Sudafrica |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Sudafrica |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Sudafrica |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="south-georgia-and-the-south-sandwich-islands"></a>Georgia del Sud e Sandwich australi
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="spain"></a>Spagna
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="sri-lanka"></a>Sri Lanka
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="suriname"></a>Suriname
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="svalbard-and-jan-mayen-islands"></a>Svalbard e Jan Mayen
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="swaziland"></a>Swaziland
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="sweden"></a>Svezia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Unione Europea |
| OneDrive for Business | Unione Europea |
| SharePoint Online | Unione Europea |
| Skype for Business | Unione Europea |
| Microsoft Teams | Unione Europea |
| Office Online &amp; Mobile | Unione Europea |
| EOP | Unione Europea |
| Intune | Unione Europea |
| MyAnalytics | Unione Europea |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea  |
| Servizi di OneNote | Unione Europea |
| Stream | Unione Europea |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="switzerland"></a>Svizzera
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Svizzera |
| OneDrive for Business | Svizzera |
| SharePoint Online | Svizzera |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Svizzera |
| Office Online &amp; Mobile | Svizzera |
| EOP | Svizzera |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Svizzera |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Svizzera |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Area geografica globale 1 - EMEA |
| Moduli | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="taiwan"></a>Taiwan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="tajikistan"></a>Tagikistan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="united-republic-of-tanzania"></a>Tanzania
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="thailand"></a>Thailandia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="timor-leste"></a>Timor Est
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="togo"></a>Togo
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="tokelau"></a>Tokelau
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="tonga"></a>Tonga
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="trinidad-and-tobago"></a>Trinidad e Tobago
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="tunisia"></a>Tunisia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="turkey"></a>Turchia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="turkmenistan"></a>Turkmenistan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="turks-and-caicos-islands"></a>Isole Turks e Caicos
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="tuvalu"></a>Tuvalu
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="us-virgin-islands"></a>Isole Vergini Americane
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="uganda"></a>Uganda
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="ukraine"></a>Ucraina
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="united-arab-emirates"></a>Emirati Arabi Uniti
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Emirati Arabi Uniti |
| OneDrive for Business | Emirati Arabi Uniti |
| SharePoint Online | Emirati Arabi Uniti |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Emirati Arabi Uniti |
| Office Online &amp; Mobile | Emirati Arabi Uniti |
| EOP | Emirati Arabi Uniti |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Emirati Arabi Uniti |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Emirati Arabi Uniti |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="united-kingdom"></a>Regno Unito
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Regno Unito |
| OneDrive for Business | Regno Unito |
| SharePoint Online | Regno Unito |
| Skype for Business | Regno Unito |
| Microsoft Teams | Regno Unito |
| Office Online &amp; Mobile | Regno Unito |
| EOP | Regno Unito |
| Intune | Unione Europea |
| MyAnalytics | Regno Unito |
| Planner | Unione Europea |
| Sway | Stati Uniti |
| Yammer | Unione Europea |
| Servizi di OneNote | Regno Unito |
| Stream | Regno Unito |
| Whiteboard | Unione Europea |
| Forms | Unione Europea |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="united-states"></a>Stati Uniti
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="uruguay"></a>Uruguay
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 3 - Americhe |
| OneDrive for Business | Area geografica globale 3 - Americhe |
| SharePoint Online | Area geografica globale 3 - Americhe |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Area geografica globale 3 - Americhe |
| EOP | Area geografica globale 3 - Americhe |
| Intune | Stati Uniti |
| MyAnalytics | Area geografica globale 3 - Americhe |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 3 - Americhe |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="us-minor-outlying-islands"></a>Altre isole americane del Pacifico
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="uzbekistan"></a>Uzbekistan
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="vanuatu"></a>Vanuatu
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="venezuela"></a>Venezuela
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Stati Uniti |
| OneDrive for Business | Stati Uniti |
| SharePoint Online | Stati Uniti |
| Skype for Business | Stati Uniti |
| Microsoft Teams | Stati Uniti |
| Office Online &amp; Mobile | Stati Uniti |
| EOP | Stati Uniti |
| Intune | Stati Uniti |
| MyAnalytics | Stati Uniti |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Stati Uniti |
| Stream | Stati Uniti |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="vietnam"></a>Vietnam
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="wallis-and-futuna-islands"></a>Wallis e Futuna
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 2 - Asia Pacifico |
| OneDrive for Business | Area geografica globale 2 - Asia Pacifico |
| SharePoint Online | Area geografica globale 2 - Asia Pacifico |
| Skype for Business | Area geografica globale 2 - Asia Pacifico |
| Microsoft Teams | Area geografica globale 2 - Asia Pacifico |
| Office Online &amp; Mobile | Area geografica globale 2 - Asia Pacifico |
| EOP | Area geografica globale 2 - Asia Pacifico |
| Intune | Area geografica globale 2 - Asia Pacifico |
| MyAnalytics | Area geografica globale 2 - Asia Pacifico |
| Planner | Area geografica globale 2 - Asia Pacifico |
| Sway | Stati Uniti |
| Yammer | Stati Uniti |
| Servizi di OneNote | Area geografica globale 2 - Asia Pacifico |
| Stream | Area geografica globale 2 - Asia Pacifico |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="western-sahara"></a>Sahara Occidentale
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="yemen"></a>Yemen
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Area geografica globale 1 - EMEA |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Area geografica globale 1 - EMEA |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="zambia"></a>Zambia
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>

## <a name="zimbabwe"></a>Zimbabwe
<details><summary>Fare clic per espandere</summary><p>

| Servizio | Posizione |
| --- | --- |
| Exchange Online | Area geografica globale 1 - EMEA |
| OneDrive for Business | Area geografica globale 1 - EMEA |
| SharePoint Online | Area geografica globale 1 - EMEA |
| Skype for Business | Area geografica globale 1 - EMEA |
| Microsoft Teams | Area geografica globale 1 - EMEA |
| Office Online &amp; Mobile | Area geografica globale 1 - EMEA |
| EOP | Area geografica globale 1 - EMEA |
| Intune | Area geografica globale 1 - EMEA |
| MyAnalytics | Area geografica globale 1 - EMEA |
| Planner | Stati Uniti |
| Sway | Stati Uniti |
| Yammer | Area geografica globale 1 - EMEA |
| Servizi di OneNote | Area geografica globale 1 - EMEA |
| Stream | Area geografica globale 1 - EMEA |
| Whiteboard | Stati Uniti |
| Forms | Stati Uniti |
| Workplace Analytics | Stati Uniti |
</p></details>
