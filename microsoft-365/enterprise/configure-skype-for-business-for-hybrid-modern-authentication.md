...
L'autenticazione moderna, è un metodo di gestione delle identità che offre un'autenticazione e un'autorizzazione degli utenti più sicure Skype for Business, è disponibile per i server locali e Exchange server locali e ibridi Skype for Business dominio diviso.
...
1. Raccogliere le informazioni specifiche di HMA necessarie in un file o OneNote.
...
1. Attivare l'autenticazione moderna ibrida Exchange locale.

1. Attivare l'autenticazione moderna ibrida Skype for Business locale.
...
Vedere [l'argomento Supportability per Skype for Business con MA](/skypeforbusiness/plan-your-deployment/modern-authentication/topologies-supported) per le topologie supportate.
...
- **Tenant ID**
  ...
  - GUID che rappresenta il tenant Office 365 (all'accesso di contoso.onmicrosoft.com).
...
Se si utilizza un server edizione Standard, l'URL interno sarà vuoto. In questo caso, utilizzare il nome di dominio completo del pool per l'URL interno.
...
## <a name="turn-on-hybrid-modern-authentication-for-exchange-on-premises"></a>Attivare l'autenticazione moderna ibrida Exchange locale
...
## <a name="turn-on-hybrid-modern-authentication-for-skype-for-business-on-premises"></a>Attivare l'autenticazione moderna ibrida Skype for Business locale
...
1. Innanzitutto, connettersi a Azure Active Directory (Azure AD) con [queste istruzioni.](/powershell/azure/active-directory/overview)
...
2. Eseguire questo comando, in locale, per ottenere un elenco degli URL del servizio Web SFB.
   ...
   Prendere nota (e screenshot per un confronto successivo) dell'output di questo comando, che includerà un URL edizione Standard e WS, ma costituito principalmente da SPN che iniziano con `00000004-0000-0ff1-ce00-000000000000/` .
...
È necessario sapere come usare l'autenticazione moderna (ADAL) per i Skype for Business client? Di seguito sono riportati i [passaggi.](./hybrid-modern-auth-overview.md)