...
description: In questo articolo vengono elencate le specifiche del dispositivo utilizzate in Microsoft Managed Desktop.
...
Microsoft Managed Desktop beneficia degli utenti che assicurano che utilizzino sempre dispositivi che offrono le migliori funzionalità di prestazioni, affidabilità, progettazione e sicurezza (come il supporto di funzionalità come Windows Hello). Per ottenere questo risultato, Microsoft Managed Desktop gestisce un breve catalogo di [dispositivi approvati](device-list.md)continuamente aggiornati. 
 
In questo articolo vengono illustrati in dettaglio il ciclo di vita dei dispositivi che vengono aggiunti e rimossi dal catalogo approvato. 
...
I prodotti rimangono nel catalogo per un massimo di 24 mesi, ma i <em>dispositivi</em> restano in gestione per tre anni in base alle singole date di registrazione. In effetti, ogni prodotto ha tre date importanti, ma ogni dispositivo ne ha uno solo. Per i prodotti, tutte e tre le date vengono calcolate in base alla <em>Data di approvazione</em>e pertanto si pubblicano queste date al momento dell'approvazione, in modo da poter sempre guardare avanti e pianificare adeguatamente l'intero ciclo di vita del prodotto.
...
Durante questa fase, il team di ingegneri di Microsoft Managed Desktop esegue la valutazione e la certificazione di un prodotto. Il team valuta elementi quali l'affidabilità e le prestazioni di Windows, la conformità con una linea di base hardware, il sentimento di mercato e l'inventario e la preparazione dei canali, tra le altre cose. Questo processo in genere richiede circa sei settimane.
  
Microsoft Managed Desktop valuterà solo i dispositivi per la certificazione entro i primi sei mesi di disponibilità. Questo criterio garantisce che si stia sempre concentrando gli sforzi sull'hardware di ultima generazione.
...
È possibile pensare a questo periodo in modo efficace "registrazione aperta", in modo da massimizzare il valore di Microsoft Managed Desktop, è consigliabile indirizzare i modelli di acquisizione e i prodotti selezionati entro questo periodo. Come piccolo esempio, è consigliabile evitare di accontentarsi di un periodo di due anni per il roll-out utilizzando un prodotto che si trova nell'ultimo mese di disponibilità principale: la maggior parte di questi dispositivi non riceverà i tre anni completi di Microsoft Managed Desktop Management (vedere il [periodo di tolleranza](#product-grace-period) per ulteriori informazioni).  
...
Se i dispositivi approvati sono stati acquistati di recente prima di effettuare l'iscrizione con Microsoft Managed Desktop, è comunque possibile registrarli, ma non si riceveranno tre anni di gestione completi. Al contrario, non conformi alla data di pensionamento, indipendentemente dal momento in cui sono stati registrati. Dietro le quinte, Microsoft Managed Desktop tratterà questi dispositivi come se fossero registrati nell'ultimo giorno di disponibilità principale. In questa figura, è possibile vedere questo scenario osservando che sia il dispositivo blu che quello verde terminano nello stesso giorno, nonostante la differenza di un anno di registrazione:
...
Un dispositivo non è in conformità quando è trascorso il relativo Window consentita per la gestione di Microsoft Managed Desktop. Questa situazione si verifica quando il dispositivo ha raggiunto i tre anni di gestione o quando questo tipo di prodotto viene rimosso dal Catalogo dispositivi, a seconda di quale si verifica per primo. È consigliabile utilizzare sempre i cicli di acquisizione in modo che i nuovi dispositivi vengano distribuiti prima che i dispositivi correnti siano fuori conformità.

Il team di Microsoft Managed Desktop sa che i cicli di approvvigionamento sono lunghi e pianificati in merito ai budget a esecuzione prolungata. Per assicurarsi di essere sempre a conoscenza dello stato della popolazione del dispositivo, viene fornito un [sito Web](https://aka.ms/mmdportal) che elenca tutti i dispositivi in gestione, la sua età e uno stato che ne indica la conformità. Il sito Web consente di avere sempre le informazioni più aggiornate sull'età del dispositivo e può utilizzare il report in qualsiasi ciclo di pianificazione degli appalti. 
