...
| `ConfidenceLevel` | stringa | Elenco dei livelli di sicurezza di eventuali verdetti di posta indesiderata o phishing. Per la posta indesiderata, questa colonna mostra il livello di probabilità di posta indesiderata (SCL), che indica se il messaggio di posta elettronica è stato ignorato (-1), non è stato trovato come posta indesiderata (0,1), è stato trovato come posta indesiderata con confidenza moderata (5,6) o è stato trovato come posta indesiderata con alta probabilità (9). Per il phishing, questa colonna indica se il livello di probabilità è "Alto" o "Basso". |
