...
||[Gli utenti a rischio elevato devono modificare la password](#high-risk-users-must-change-password)|Forza gli utenti a cambiare la password quando a loro volta a loro volta si accede se vengono rilevate attività ad alto rischio per il proprio account.|
...
||Exclude|**Utenti e gruppi**: selezionare il gruppo di eccezioni di accesso condizionale; account di servizio (identità delle app).|L'appartenenza deve essere modificata in base alle esigenze e temporaneamente.|
...
Per **le proprietà del** dispositivo, specificare i valori appropriati per le versioni del sistema operativo in base ai criteri di sicurezza e IT.
...
||Antispyware|Require|Select <p> Questa impostazione richiede una soluzione anti-spyware registrata nel Centro sicurezza PC Windows.|
