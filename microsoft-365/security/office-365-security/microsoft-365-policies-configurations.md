...
Questa serie di articoli descrive un set di configurazioni dei prerequisiti per l'accesso a identità e dispositivi e un set di Azure Active Directory (Azure AD) Accesso condizionale, Microsoft Intune e altri criteri per proteggere l'accesso a Microsoft 365 per app e servizi cloud aziendali, altri servizi SaaS e applicazioni locali pubblicate con proxy di applicazione Azure AD.
...
- Sono allineati a [Microsoft Secure Score](../defender/microsoft-secure-score.md) e al punteggio di identità in Azure [AD](/azure/active-directory/fundamentals/identity-secure-score)e aumenteranno questi punteggi per l'organizzazione.
...
> [!NOTE]
> Microsoft vende anche licenze Enterprise Mobility + Security (EMS) per Office 365 abbonamenti. Le funzionalità EMS E3 ed EMS E5 sono equivalenti a quelle in Microsoft 365 E3 e Microsoft 365 E5. Per [i dettagli, vedere Piani EMS.](https://www.microsoft.com/microsoft-365/enterprise-mobility-security/compare-plans-and-pricing)

## <a name="intended-audience"></a>Destinatario previsto

Questi suggerimenti sono destinati agli architetti aziendali e ai professionisti IT che hanno familiarità con i servizi di sicurezza e produttività cloud di Microsoft 365, che includono Azure AD (identità), Microsoft Intune (gestione dei dispositivi) e Microsoft Information Protection (protezione dei dati).
...
Molti dei suggerimenti forniti si basano su servizi disponibili solo con le licenze Microsoft 365 E5, Microsoft 365 E3 con il componente aggiuntivo E5 Security, EMS E5 o Azure AD Premium P2.

Per le organizzazioni che non dispongono di queste licenze, Microsoft consiglia di implementare almeno le impostazioni predefinite di [sicurezza,](/azure/active-directory/fundamentals/concept-fundamentals-security-defaults)incluse in tutti Microsoft 365 piani.
...
Il **modello identity and device protection for Microsoft 365** architecture mostra quali funzionalità sono paragonabili.

[![Immagine thumb per identity e protezione del dispositivo per Microsoft 365 poster](../../media/microsoft-365-policies-configurations/o365-identity-device-protection-thumb.png)](../../downloads/MSFT_cloud_architecture_identity&device_protection.pdf) <br> [Visualizzazione in formato PDF](../../downloads/MSFT_cloud_architecture_identity&device_protection.pdf) \| [Download in formato PDF](https://github.com/MicrosoftDocs/microsoft-365-docs/raw/public/microsoft-365/downloads/MSFT_cloud_architecture_identity&device_protection.pdf) \| [Scarica come Visio](https://github.com/MicrosoftDocs/microsoft-365-docs/raw/public/microsoft-365/downloads/MSFT_cloud_architecture_identity&device_protection.vsdx)  

Vedere inoltre la soluzione Distribuire la protezione delle informazioni per le normative sulla [privacy](../../solutions/information-protection-deploy.md) dei dati per proteggere le informazioni archiviate in Microsoft 365.
...
In questa sezione viene fornita una panoramica dei servizi Microsoft 365 e delle funzionalità importanti per l'identità e l'accesso ai dispositivi.
...
[Intune](/intune/introduction-intune) è il servizio di gestione dei dispositivi mobili basato su cloud di Microsoft. Questa guida consiglia la gestione dei dispositivi Windows PC con Intune e consiglia le configurazioni dei criteri di conformità dei dispositivi. Intune determina se i dispositivi sono conformi e invia questi dati ad Azure AD per l'utilizzo durante l'applicazione dei criteri di accesso condizionale.
...
- [Roadmap per la sicurezza: priorità principali per i primi 30 giorni, 90 giorni e oltre](security-roadmap.md)
  ...
  Consigli che includono la registrazione, la governance dei dati, l'accesso amministratore e la protezione dalle minacce.

### <a name="windows-10-and-microsoft-365-apps-for-enterprise"></a>Windows 10 e App Microsoft 365 per grandi imprese

Windows 10 con Microsoft 365 Apps for enterprise è l'ambiente client consigliato per i PC. È consigliabile Windows 10 perché Azure è progettato per offrire l'esperienza più fluida possibile sia per l'ambiente locale che per Azure AD. Windows 10 include anche funzionalità di sicurezza avanzate che possono essere gestite tramite Intune. Microsoft 365 Apps for enterprise include le versioni più recenti delle Office applicazioni. Questi utilizzano l'autenticazione moderna, che è più sicura e un requisito per l'accesso condizionale. Queste app includono anche strumenti di conformità e sicurezza avanzati.
...
La tabella sopra riportata riflette la tendenza di molte organizzazioni a supportare una combinazione di dispositivi di proprietà dell'organizzazione, nonché di dispositivi personali o BYOD per consentire la produttività mobile tra la forza lavoro. I criteri di protezione delle app di Intune garantiscono che la posta elettronica sia protetta dall'esfiltrazione dall'app per dispositivi mobili Outlook e da altre app per dispositivi mobili Office, sia nei dispositivi di proprietà dell'organizzazione che nei BYOD.
...
- Analizza questo elenco di app per determinare i set di criteri che forniscono livelli di protezione appropriati.
  ...
  Ad esempio, potresti avere un set di criteri che includono tutte le app Microsoft 365 per tutti gli utenti per la protezione di base e un secondo set di criteri per tutte le app sensibili, ad esempio quelle usate da risorse umane o reparti finanziari, e applicarle a tali gruppi.
...
Ad esempio, configura i criteri che verranno usati per tutte le app Microsoft 365 solo per Exchange Online con le modifiche aggiuntive per Exchange. Implementare questi criteri agli utenti e risolvere eventuali problemi. Quindi, aggiungere Teams con le modifiche aggiuntive e implementare questo agli utenti. Quindi, aggiungere SharePoint con le modifiche aggiuntive. Continua ad aggiungere il resto delle tue app fino a quando non puoi configurare con sicurezza questi criteri di base per includere tutte Microsoft 365 app.
...
## <a name="steps-to-configure-identity-and-device-access"></a>Passaggi per configurare l'identità e l'accesso al dispositivo
...
4. Configurare i criteri di accesso condizionale Microsoft 365 app cloud,ad esempio Microsoft Teams, Exchange Online e SharePoint Microsoft Cloud App Security.e criteri di Microsoft Cloud App Security.
