...
- Il record MX del dominio è stato modificato di recente e i server DNS hanno ancora memorizzato nella cache le informazioni DNS per il dominio.

### <a name="how-do-i-fix-error-code-450-44312"></a>Come si risolve il codice di errore 450 4.4.312?

- Collaborare con il servizio di hosting DNS per identificare e risolvere il problema relativo al dominio.
...
In genere, ciò significa che Microsoft 365 non è in grado di connettersi al server di posta elettronica di destinazione. Nei dettagli dell'errore è riportata una spiegazione del problema. Ad esempio:
...
### <a name="how-do-i-fix-error-code-450-44315"></a>Come si risolve il codice di errore 450 4.4.315?
...
In genere, questo errore indica che Microsoft 365 ha riscontrato un errore di connessione quando ha tentato di connettersi al server di posta elettronica di destinazione. Una probabile causa di questo errore è che il firewall blocca le connessioni dagli indirizzi IP di Microsoft 365. Oppure, questo errore potrebbe essere da progettazione se è stata eseguita la migrazione completa del sistema di posta elettronica locale a Microsoft 365 e si arresta l'ambiente di posta elettronica locale.
...
- Se non devono essere recapitati altri messaggi all'ambiente locale, fare clic su Correggi ora nell'avviso **in** modo che Microsoft 365 possa rifiutare immediatamente i messaggi con destinatari non validi. Questo consente di ridurre il rischio di superare la quota dell'organizzazione per i destinatari non validi, che possono compromettere il recapito dei messaggi normali. In alternativa, è possibile utilizzare le istruzioni seguenti per risolvere manualmente il problema:
  ...
  - Nell'interfaccia di amministrazione [di Exchange disabilitare](https://docs.microsoft.com/Exchange/exchange-admin-center)o eliminare il connettore che recapita la posta elettronica da Microsoft 365 all'ambiente di posta elettronica locale:

    1. Nell'interfaccia di amministrazione di Exchange, andare a **Connettori del flusso** \> **di posta.**

    2. Selezionare il connettore con il valore **Da** **Office 365** e il valore **A** per il server di posta elettronica **dell'organizzazione** ed eseguire una delle operazioni seguenti:

       - Eliminare il connettore facendo clic **sull'icona** ![ Elimina](../../media/adf01106-cc79-475c-8673-065371c1897b.gif)

       - Disabilitare il connettore facendo **clic sull'icona** ![ Modifica e ](../../media/ebd260e4-3556-4fb0-b0bb-cc489773042c.gif) **deselezionaNdo Attiva.**

  - Modificare il dominio accettato in Microsoft 365 associato all'ambiente di posta elettronica locale da **Inoltro** interno **a Autorevole.** Per istruzioni, vedere [Gestire i domini accettati in Exchange Online.](https://docs.microsoft.com/exchange/mail-flow-best-practices/manage-accepted-domains/manage-accepted-domains)

  **Nota:** in genere, per l'applicazione di queste modifiche sono necessarie tra 30 minuti e un'ora. Dopo un'ora, verificare che l'errore non venga più visualizzato.
...
In genere, questo errore indica che Microsoft 365 si è connesso al server di posta elettronica di destinazione, ma il server ha risposto con un errore immediato o non soddisfa i requisiti di connessione. Nei dettagli dell'errore è riportata una spiegazione del problema. Ad esempio:
...
### <a name="how-do-i-fix-error-code-450-44317"></a>Come si risolve il codice di errore 450 4.4.317?
...
In genere, questo errore indica che Microsoft 365 ha difficoltà a comunicare con l'ambiente di posta elettronica locale, quindi la connessione è stata interrotta. Alcune possibili cause di questo errore:

- Il firewall utilizza le regole di verifica del pacchetto SMTP e tali regole non funzionano correttamente.

- Il server di posta elettronica locale non funziona correttamente (ad esempio, il servizio si blocca, si arresta in modo anomalo o risorse di sistema limitate), causando il timeout del server e chiudere la connessione a Microsoft 365.

- Esistono problemi di rete tra l'ambiente locale e Microsoft 365.

### <a name="how-do-i-fix-error-code-450-44318"></a>Come si risolve il codice di errore 450 4.4.318?
...
### <a name="how-do-i-fix-error-code-450-47320"></a>Come si risolve il codice di errore 450 4.7.320?

- Correggere il certificato o le impostazioni del connettore in modo che i messaggi in coda in Microsoft 365 possano essere recapitati.

- Se l'errore è generato nell'organizzazione del partner (ad esempio, il provider di servizi cloud di terze parti), è necessario contattarlo e chiedere la risoluzione del problema.

## <a name="other-error-codes"></a>Altri codici errore

Microsoft 365 ha difficoltà a recapitare i messaggi al server di posta elettronica locale o del partner. Utilizzare le informazioni **Server di destinazione** nell'errore per esaminare l'errore nel proprio ambiente o per modificare il connettore in presenza di un errore di configurazione.
