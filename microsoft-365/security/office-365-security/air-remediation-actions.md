...
|Posta elettronica|URL dannoso <br> (Un URL dannoso è stato rilevato [da Cassaforte .)](safe-links.md)|Eliminazione recidiva della posta elettronica/cluster <br> URL di blocco (verifica del momento del clic) <p> La posta elettronica che contiene un URL dannoso è considerata dannosa.|
...
|Utente|Un utente ha fatto clic su un URL dannoso <br> Un utente si è connesso a una pagina che in seguito è stata trovata dannosa oppure un utente ha ignorato una pagina di avviso dei collegamenti [di Cassaforte](safe-links.md#warning-pages-from-safe-links) per accedere a una pagina dannosa.|L'indagine automatizzata non comporta un'azione in sospeso specifica. <p> Blocco di URL (al momento del clic) <p> Usa Esplora minacce per [visualizzare i dati sugli URL e fai clic su verdetti.](threat-explorer.md#view-phishing-url-and-click-verdict-data) <p> Se l'organizzazione usa [Microsoft Defender for Endpoint,](/windows/security/threat-protection/)valuta la possibilità di analizzare l'utente per determinare se il suo account è compromesso. [](/microsoft-365/security/defender-endpoint/investigate-user)|
