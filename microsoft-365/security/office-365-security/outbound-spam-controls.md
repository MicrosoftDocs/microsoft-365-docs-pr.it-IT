...
Nelle Microsoft 365 con cassette postali in Exchange Online o in organizzazioni di Exchange Online Protection (EOP) autonome senza cassette postali Exchange Online, la gestione della posta indesiderata in uscita è molto importante. Anche se un cliente invia intenzionalmente o involontariamente posta indesiderata dall'organizzazione, tale azione può compromettere la reputazione dell'intero servizio e influire sul recapito della posta elettronica per altri clienti.

In questo articolo vengono descritti i controlli e le notifiche progettati per impedire la posta indesiderata in uscita e le attività che è possibile eseguire se è necessario inviare messaggi di posta elettronica di massa.
...
- **Monitoraggio della reputazione degli indirizzi IP di** origine: Microsoft 365 query su vari elenchi di indirizzi IP di terze parti. Se uno degli indirizzi IP utilizzati per la posta elettronica in uscita viene visualizzato in questi elenchi, viene generato un avviso. Questo monitoraggio ci consente di reagire rapidamente quando la posta indesiderata ha causato la degradazione della reputazione. Quando viene generato un avviso, è disponibile una documentazione interna che descrive come rimuovere (eliminare) gli indirizzi IP dagli elenchi di blocco.
...
- **Inviare messaggi di posta elettronica in** blocco tramite server di posta elettronica locali: i clienti mantengono la propria infrastruttura di posta elettronica per gli invii di massa.
