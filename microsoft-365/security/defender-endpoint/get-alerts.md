...
Recupera una raccolta di avvisi.
<br>Supporta le [query OData V4.](https://www.odata.org/documentation/)
<br>Operatori supportati da OData:
<br>```$filter``` on: ```alertCreationTime``` ```lastUpdateTime``` , , , , ```incidentId``` e ```InvestigationId``` le ```status``` ```severity``` ```category``` proprietà.
<br>```$top``` con valore massimo di 10.000
<br>```$skip```
<br>```$expand``` di ```evidence```
