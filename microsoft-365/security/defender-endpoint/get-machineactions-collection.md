...
Recupera un insieme di [azioni del computer.](machineaction.md)
<br>Supporta le [query OData V4.](https://www.odata.org/documentation/)
<br>La query di OData ```$filter``` è supportata su: ```status``` , , e sulle ```machineId``` ```type``` ```requestor``` ```creationDateTimeUtc``` proprietà.
...
Se ha esito positivo, questo metodo restituisce 200 codice di risposta Ok con una raccolta di [entità machineAction.](machineaction.md)
...
Ecco un esempio della richiesta in un'organizzazione con tre MachineActions.
...
Ecco un esempio di richiesta che filtra MachineActions in base all'ID computer e mostra le ultime due MachineActions.
