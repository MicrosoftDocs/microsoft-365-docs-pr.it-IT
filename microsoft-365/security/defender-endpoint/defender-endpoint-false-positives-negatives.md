...
Fortunatamente, è possibile eseguire operazioni per risolvere e ridurre questo tipo di problemi. Se nel Microsoft [Defender Security Center](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/use)vengono visualizzati falsi positivi/negativi, le operazioni di sicurezza possono adottare misure per affrontarli utilizzando il processo seguente:
...
Inoltre, è possibile ottenere assistenza se si verificano ancora problemi con [falsi positivi/negativi](#still-need-help) dopo aver eseguito le attività descritte in questo articolo.
...
3. Selezionare un avviso per ulteriori dettagli sull'avviso. (Vedere [Esaminare gli avvisi.)](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/review-alerts)
...
> [!TIP]
> Serve assistenza per le regole di eliminazione? Vedere [Eliminazione di un avviso e creazione di una nuova regola di eliminazione.](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/manage-alerts#suppress-an-alert-and-create-a-new-suppression-rule)
...
> [!NOTE]
> Le esclusioni di Microsoft Defender Antivirus si applicano solo alla protezione antivirus e non ad altre funzionalità di Microsoft Defender for Endpoint. Per escludere i file in generale, usa le esclusioni per Microsoft Defender Antivirus e [gli indicatori personalizzati](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/manage-indicators) per Microsoft Defender per Endpoint.
...
> [!TIP]
> Serve assistenza per le esclusioni antivirus? Vedi [Configurare e convalidare le esclusioni per le analisi di Microsoft Defender Antivirus.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/configure-exclusions-microsoft-defender-antivirus)
...
7. Nella scheda **Tag ambito,** se si utilizzano tag di ambito nell'organizzazione, specificare i tag di ambito per il criterio che si sta creando. Vedere [Tag di ambito.](https://docs.microsoft.com/mem/intune/fundamentals/scope-tags)
8. Nella scheda **Assegnazioni** specificare gli utenti e i gruppi a cui applicare i criteri e quindi scegliere **Avanti**. Per assistenza con le assegnazioni, vedere [Assegnare profili utente e dispositivo in Microsoft Intune.](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign)
...
Quando si [crea un indicatore "Consenti"](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-file)per un file, ad esempio un file eseguibile, si evita che i file che l'organizzazione sta utilizzando vengano bloccati. I file possono includere file eseguibili portabili (PE), ad esempio `.exe` e `.dll` file. 

Prima di creare indicatori per i file, verificare che siano soddisfatti i requisiti seguenti:
- Microsoft Defender Antivirus è configurato con la protezione basata su cloud abilitata (vedere [Gestire la protezione basata su cloud)](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/deploy-manage-report-microsoft-defender-antivirus)
...
- La [funzionalità Blocca o consenti è attivata](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/advanced-features) 

#### <a name="indicators-for-ip-addresses-urls-or-domains"></a>Indicatori per indirizzi IP, URL o domini

Quando si [crea un indicatore "Consenti"](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-ip-domain)per un indirizzo IP, un URL o un dominio, si evita che i siti o gli indirizzi IP utilizzati dall'organizzazione vengano bloccati.

Prima di creare indicatori per indirizzi IP, URL o domini, verificare che siano soddisfatti i requisiti seguenti:
- La protezione di rete in Defender for Endpoint è abilitata in modalità blocco (vedere [Abilitare la protezione di rete](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/enable-network-protection))
...
Gli indicatori di rete personalizzati sono attivati in Microsoft Defender Security Center (vedi [Funzionalità avanzate)](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/advanced-features)   
...
1. Rivedere le linee guida qui: [Inviare file per l'analisi](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide).
...
3.  Rivedere le linee guida qui: [Inviare file per l'analisi](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide).
...
> [!TIP]
> Per ulteriori informazioni, vedere [Inviare file per l'analisi.](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide#how-does-microsoft-prioritize-submissions)
...
> [!TIP]
> Per ulteriori informazioni sulla configurazione della protezione basata sul cloud, vedere [Specify the cloud-delivered protection level.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/specify-cloud-protection-level-microsoft-defender-antivirus)
...
6. Nella scheda **Tag ambito,** se si utilizzano tag di ambito nell'organizzazione, specificare i tag di ambito per il criterio. Vedere [Tag di ambito.](https://docs.microsoft.com/mem/intune/fundamentals/scope-tags)
8. Nella scheda **Assegnazioni** specificare gli utenti e i gruppi a cui applicare i criteri e quindi scegliere **Avanti**. Per assistenza con le assegnazioni, vedere [Assegnare profili utente e dispositivo in Microsoft Intune.](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign)
9. Nella scheda **Revisione e creazione** rivedere le impostazioni e quindi scegliere **Crea.**  
...
> [!TIP]
> Per ulteriori informazioni sulla puA, vedere [Detect and block potentially unwanted applications](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/detect-block-potentially-unwanted-apps-microsoft-defender-antivirus).
...
7. Nella scheda **Assegnazioni** specificare gli utenti e i gruppi a cui applicare i criteri e quindi scegliere **Avanti**. Per assistenza con le assegnazioni, vedere [Assegnare profili utente e dispositivo in Microsoft Intune.](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign)
